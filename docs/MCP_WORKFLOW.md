...
| **세션 상태**            | `session_status`                                                                                                                                                                                                 | 작업 재개 전 현재 문서, dirty/autosave, 복구, 스냅샷 상태 확인                          |
...
2. **세션 상태 확인** — 중단된 작업을 재개하거나 위험한 수정 전에는 `session_status`로 현재 문서/dirty/autosave/복구 상태를 먼저 확인
3. **좁게 읽기** — 필요한 항목/섹션/범위만 읽기
4. **targeted edit** — 작은 필드는 전체 교체, 큰 필드는 검색 후 부분 교체
5. **batch 우선** — 이웃한 여러 항목을 수정할 땐 batch 도구를 우선 사용
6. **검증** — CBS/태그/참고 자료 비교/프리뷰로 결과 확인

### 상황별 빠른 선택

**필드 내용을 확인하고 싶을 때**

- 현재 세션/문서 상태 확인 → `session_status`
- 소형 필드 → `read_field`
...
- 작업 재개 전 dirty/autosave/복구 상태 점검 → `session_status`
...
- `session_status`는 활성 문서가 없어도 호출 가능하며, 현재 파일 경로/타입, renderer dirty/autosave 상태, pending recovery, snapshot totals를 한 번에 보여줍니다.
