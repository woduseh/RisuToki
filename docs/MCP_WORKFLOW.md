...
| **Session state**             | `session_status`, `save_current_file`                                                                                                                                                                                                                                                                                                                                                                                                                          | Check current document, dirty/autosave, recovery, snapshot state, compact structured-surface counts, or explicitly save after MCP edits                                 |
| **Surface fallback**          | `list_surfaces`, `read_surface`, `patch_surface`, `replace_in_surface`                                                                                                                                                                                                                                                                                                                                                                                         | JSON Pointer based fallback reads/edits when specialized active-document tools cannot reach the required `.charx` / `.risum` / `.risup` content                         |
| **External file read/write**  | `inspect_external_file`, `probe_field`, `probe_field_batch`, `probe_lorebook`, `probe_regex`, `probe_lua`, `probe_css`, `probe_greetings`, `probe_triggers`, `probe_risup_prompt_items`, `probe_risup_formating_order`, `external_search_in_field`, `external_read_field_range`, `external_write_field`, `external_write_field_batch`, `external_replace_in_field`, `external_insert_in_field`, `external_read_surface`, `external_patch_surface`, `open_file` | Inspect or edit `.charx` / `.risum` / `.risup` files by absolute path without switching the active UI document, or explicitly switch the active document when needed    |
...
- **Read unopened files with `inspect_external_file` + `probe_*` first.** If you must keep the current UI document untouched, use `external_*` for direct path-based edits. Switch the active document with `open_file` only when you specifically need the live session-coupled edit families.
...
- File not open, identified by absolute path → `inspect_external_file` / relevant `probe_*` readers / `external_search_in_field` / `external_read_field_range`
...
- File not yet open but current UI document must stay unchanged → `external_write_field` / `external_write_field_batch` / `external_replace_in_field` / `external_insert_in_field` / `external_read_surface` / `external_patch_surface`
- Specialized active-document tool cannot reach the required content → `list_surfaces` → `read_surface` → `patch_surface` or `replace_in_surface` with `dry_run` first when practical
- File not yet open and you want the full active-document editing surface → switch with `open_file(file_path=...)`, then use existing `write_*` / `replace_*` tools
