...
| ----------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Lorebook cleanup**          | `inspect_document` → `search_document` / bounded `read_content`; use `list_lorebook` → `read_lorebook_batch` when entry metadata, folders, or stale-index guards matter                                      | `replace_in_lorebook_batch(dry_run=true)` for multi-entry text changes, or `preview_edit` when the covered field/surface selector is enough                                          | `write_lorebook_batch`, `replace_in_lorebook_batch`, `insert_in_lorebook_batch`, or `batch_delete_lorebook` with `expected_comment` / `expected_comments`; use `apply_edit` only for covered facade previews   | `validate_lorebook_keys`, follow-up `read_lorebook_batch` / `read_content`, and check Lua `getLoreBooks()` comment searches if comments changed                      |
| **Regex or greeting edits**   | `inspect_document`; then facade `read_content`, `list_regex` / `read_regex_batch`, or `list_greetings` / `read_greeting_batch` instead of dumping raw arrays                                                 | For single indexed write/delete use `preview_edit` → `apply_edit`; use granular dry-run/preview routes for broad text replacement                                                    | `replace_in_regex`, `write_regex_batch`, `batch_write_greeting`, `batch_delete_greeting`, or add/reorder routes with `expected_comment` / `expected_preview`                                                   | Re-read the edited index or list after deletion, preview affected chat output when relevant, and confirm no stale-index `409` required a refresh                     |
| **`.risup` prompt edits**     | `inspect_document`, facade `read_content`, `list_risup_prompt_items`, `search_in_risup_prompt_items`, `read_risup_prompt_item_batch`, `read_risup_formating_order`, and `diff_risup_prompt` when available   | For single indexed prompt-item write/delete use `preview_edit` → `apply_edit`; use `import_risup_prompt_from_text(dry_run=true)` or snippet insertion `dry_run` before large imports | `write_risup_prompt_item_batch`, `add_risup_prompt_item_batch`, `batch_delete_risup_prompt_items`, `import_risup_prompt_from_text`, or `insert_risup_prompt_snippet` with `expected_type` / `expected_preview` | `validate_content`, `validate_risup_prompt_import` with the same source text after imports, `diff_risup_prompt`, `read_risup_formating_order`, and targeted re-reads |
| **CBS / Danbooru validation** | Load syntax guidance first (`read_skill("writing-cbs-syntax")`, `read_skill("writing-danbooru-tags")`) and locate candidate text with `search_document` / bounded `read_content`                             | Run `validate_cbs`, `simulate_cbs`, `diff_cbs`, `validate_danbooru_tags`, or `search_danbooru_tags` before changing prompts or templates                                             | Use `preview_edit` → `apply_edit` for covered field/surface edits; fall back to dedicated prompt/field/regex/lorebook tools for structured items                                                               | Re-run the same validators, and preview/render when CBS changes affect visible output                                                                                |
...
- Covered active/external field write/replace, active surface patch, active lorebook text replacement, or active indexed regex/greeting/risup prompt item write/delete → `preview_edit` → `apply_edit` with the returned `preview_token`, `operation_digest`, and guard values.
...
- Post-v0.69.3 facade backlog: extend mutating facade parity beyond single indexed regex/greeting/risup prompt item writes/deletes into batch/add/reorder workflows where guard alignment is clear, and broaden `validate_content` only where facade selectors have clear validator parity. Current `validate_content` coverage is active lorebook key hygiene plus active `.risup` promptTemplate/formatingOrder structure checks. Until then, keep using granular validators and dedicated mutation tools for unsupported cases.
