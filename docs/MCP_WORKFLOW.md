...
- File not yet open but current UI document must stay unchanged → `external_write_field` / `external_write_field_batch` / `external_replace_in_field` / `external_insert_in_field`
...
- `external_write_field`, `external_write_field_batch`, `external_replace_in_field`, and `external_insert_in_field` also trigger confirmation and reject targets that are already the active UI document.
