...
- 목표: 두 파일을 각각 5,000줄 미만으로. 슬라이스 단위로 항상 green 유지. 리뷰 가능한 커밋 크기.
...
4. **문서 동기화**: 새 `src/lib/*.ts` 모듈은 `docs/MODULE_MAP.md`에 반드시 추가 — `src/lib/doc-drift.test.ts`의 MODULE_MAP coverage 테스트가 강제한다.
5. **빌드 설정 동기화**: `mcp-api-server.ts` 계열에서 분리된 새 모듈은 `tsconfig.node-libs.json`의 `include`에 추가한다 (`npm run build:node-libs`가 `.js` 사이드카를 생성해야 headless/standalone이 동작).
...
| 1-5+ | 패밀리별 도구 등록     | `src/lib/toki-tools/<family>.ts`       | 등록 블록을 `registerXxxTools(server, deps)` 함수로 분리. 한 번에 한 패밀리씩, 여러 슬라이스로. `shouldRegisterMcpTool` 게이트와 `safeToolHandler` 래핑을 deps로 전달                                                                                                                                    |

1-5는 분량이 크므로 (facade → structured families → reference → 기타) 순으로 4~8개 커밋으로 나눈다.
...
1. `npm run lint && npm run typecheck && npm test && npm run test:evals && npm run test:mcp`
2. 응답 동일성 스팟체크: 분할 시작 **전에** 대표 라우트 응답(각 패밀리 1개 이상: list/read/write/error 경로)을 `startTestApiServer` 기반 스크립트로 덤프해 저장해 두고, 완료 후 동일 요청 결과와 diff. 차이가 있으면 해당 슬라이스를 찾아 수정.
...
- `docs/MODULE_MAP.md`는 슬라이스마다 갱신 (doc-drift 강제).
