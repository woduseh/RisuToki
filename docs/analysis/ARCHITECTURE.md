...
├──────────────────────┴──────────────────────────────────────────┤
│  MCP Stdio Server (자식 프로세스)                                │
│  toki-mcp-server.ts (~2,020 lines) → src/lib/mcp-api-server.ts │
│  ~170개 도구, 19개 패밀리, HTTP localhost 서버                    │
└─────────────────────────────────────────────────────────────────┘
```
...
두 렌더러 모두 Vue 3 + Pinia 아키텍처를 사용합니다.
...
```
toki-mcp-server.ts (stdio transport, ~2,020줄)
  └─ src/lib/mcp-api-server.ts (HTTP route handler, ~9,200줄)
       └─ src/lib/mcp-tool-taxonomy.ts (19 패밀리, ~234줄)
       └─ src/lib/mcp-response-envelope.ts (응답 포맷, ~176줄)
       └─ src/lib/mcp-search.ts (전문 검색, ~355줄)
```

`toki-mcp-server.ts`는 메인 프로세스가 자식 프로세스로 생성하는 stdio MCP 서버입니다. 내부적으로 localhost HTTP 서버(`mcp-api-server.ts`)를 띄우고, ~170개 도구를 19개 패밀리로 분류하여 노출합니다.

도구 패밀리 목록은 `mcp-tool-taxonomy.ts`가 단일 소스 오브 트루스(SSOT)로 관리하며, `ToolAnnotations`(readOnlyHint, destructiveHint 등)도 여기서 자동 패치됩니다.
...
- **MCP 서버**는 메인 프로세스 안에서 실행되지만, `src/lib/` 아래의 독립 모듈을 사용합니다. 렌더러 상태에 직접 접근하지 않고 HTTP 요청을 통해 간접 소통합니다.
...
| 모듈 | 줄 수 | 핫스팟 이유 |
|------|-------|-------------|
| **`src/lib/mcp-api-server.ts`** | **~9,200** | ~170개 HTTP 도구 엔드포인트를 단일 파일에 보유. 프로젝트 최대 파일 |
...
AI CLI (Claude/Copilot/...)
  └─ stdio ──► toki-mcp-server.ts (MCP 프로토콜 파싱)
                └─ HTTP ──► mcp-api-server.ts (도구 라우팅)
                              ├─ 읽기 도구: 렌더러 상태에서 직접 읽기
