...
- **Post-v0.69.2 follow-up backlog:** expand `validate_content` selectors beyond active lorebook keys, and add mutating facade parity for regex, greeting, and risup prompt item workflows. These are explicitly outside the v0.69.2 scope; keep using granular validators and structured mutation families until parity is implemented and documented.
