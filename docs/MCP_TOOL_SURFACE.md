...
| --------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `facade-first`  | Yes     | Implemented facade/catalog tools: `inspect_document`, `list_tool_profiles`, `read_content`, `search_document`, `preview_edit`, `apply_edit`, `validate_content`, `load_guidance` | In default compatibility mode, `tools/list` remains unfiltered; in strict mode, only facade-first tools are registered. | Use `advanced-full` when the facade cannot express the target, selector, operation, bounds, response detail, or compatibility requirement. |
...
Facade v1 now has a focused implementation for `inspect_document`, `list_tool_profiles`, `read_content`, `search_document`, `preview_edit`, `apply_edit`, `validate_content`, and `load_guidance`. These tools compose existing HTTP routes (except `list_tool_profiles`, which is taxonomy-only) and report routed_legacy, touched_targets, guard values/previews where applicable, and facade `next_actions`. `read_content` defaults to a 24KB response cap; root surface selectors return a structure overview plus continuation hints and recommended follow-up selectors unless the caller explicitly sets `selector.include_raw=true` and an explicit byte cap. `preview_edit` / `apply_edit` cover active/external field write/replace, active surface patch, active lorebook text replacement, active lorebook id writes/deletes, active indexed or identity regex/greeting writes/deletes, active and external `.risup` prompt-item write/delete by id or index, active regex/greeting/risup prompt batch writes, active greeting/risup prompt batch deletes, and active/external `.risup` prompt-item batch writes/deletes with derived stale guards. `validate_content` covers active lorebook, regex, `.charx` export compatibility via `family: "asset"` or `field: "exportCompatibility"`, CBS, Danbooru, active/external `.risup` prompt/order checks, `.risum` semantic fields, and external Plugin v3 source scans when selectors provide enough context. Granular tools remain available as stable advanced routes for reorder/add workflows, imports/exports, assets, exact legacy payloads, and unsupported facade cases.
...
3. **Unsupported facade operation:** facade edits cover active/external field write/replace, active surface patch, active lorebook text replacement, active lorebook id writes/deletes, active regex/greeting identity writes/deletes, active/external `.risup` prompt-item id/index writes/deletes, older indexed regex/greeting/risup prompt item writes/deletes, and supported prompt/greeting/regex batch shapes. Inserts, block replacements, unsupported batch semantics, add/reorder operations, asset compression, external surface patches, and item/file management stay granular.
...
| `read_content`     | read-only  | **Implemented.** Read bounded field/surface/content through explicit selectors, including active/external surfaces, reference fields/items, and external `.risup` prompt items. Root surface reads default to overview-only with a 24KB cap.                                       |
...
| `preview_edit`     | preview    | **Implemented.** Produce a dry-run/read-only preview token for active/external field replace/write, external `.risup` prompt item writes/deletes, active surface patch, active lorebook text replace, and active indexed regex/greeting/risup prompt item write/delete operations. |
| `apply_edit`       | mutating   | **Implemented.** Apply a prior `preview_edit` by `preview_token` + `operation_digest`, carrying explicit or preview-derived guard values forward, including external `.risup` prompt item stale guards.                                                                            |
