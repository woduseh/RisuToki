...
| `write_regex`, `delete_regex`, `write_greeting`, `delete_greeting`, `write_risup_prompt_item`, `delete_risup_prompt_item` for one active indexed item; covered active batch regex/greeting/risup prompt writes and greeting/risup prompt deletes | `preview_edit` then `apply_edit` with `selector.family` + `selector.index` or batch operation payloads | Implemented for active indexed and covered active batch write/delete operations with derived stale guards. | You need regex batch deletes, add/reorder, import/export/snippet workflows, unsupported batch semantics, or exact legacy response shapes.                                                      |
...
Hidden compatibility fields are preserved on disk but are not normal MCP content surfaces: deprecated `.charx` fields, `.risup` legacy prompt fields, reserved `.risum` `cjs`, and `groupOnlyGreetings` are omitted from read/search/probe/surface/facade results. Inventory/inspect/status responses may include `hiddenFieldWarnings` with field names and count/size only. `lowLevelAccess` remains visible/editable.
