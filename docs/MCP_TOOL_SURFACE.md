...
The executable profile contract lives in `src/lib/mcp-tool-taxonomy.ts`. By default, RisuToki keeps `tools/list` unfiltered for compatibility and exposes profile guidance through `_meta` plus `list_tool_profiles`. When `--tool-profile` or the RISUTOKI MCP tool profile environment variable is set, the server registers only that profile's tools; `advanced-full` is the full legacy-compatible surface.

| Profile         | Default | Included categories                                                                                                                                                                                                              | Excluded / restricted categories                                                                                                                | Escape hatch / discovery                                                                                                                   |
| --------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `facade-first`  | Yes     | Implemented facade/catalog tools: `inspect_document`, `list_tool_profiles`, `read_content`, `search_document`, `preview_edit`, `apply_edit`, `validate_content`, `load_guidance`, `manage_items`, `manage_assets`, `manage_file` | In default compatibility mode, `tools/list` remains unfiltered; in strict mode, only facade-first tools are registered.                         | Use `advanced-full` when the facade cannot express the target, selector, operation, bounds, response detail, or compatibility requirement. |
...
| `readonly`      | No      | Only tools with `readOnlyHint=true`, including the read-only first-wave facade tools                                                                                                                                             | `preview_edit`, `apply_edit`, `manage_items`, `manage_assets`, `manage_file`, every mutating/destructive tool, and session/file-open mutations. | Request `advanced-full` only after leaving read-only mode intentionally.                                                                   |
...
2. The default is `facade-first` for planning and prompt guidance, but `tools/list` still returns the full tool catalog when no strict profile is configured.
...
4. `list_tool_profiles` returns `currentProfile`, `strictFiltering`, `toolsListBehavior`, and registered/hidden counts so clients can tell whether they are seeing the compatibility surface or a strict registered subset.
5. Preferred low-context path: call `list_tool_profiles` with `profile="facade-first"` (or `authoring`/`readonly`) and use the returned names for planning. Request `profile="advanced-full"` or alias `advanced`/`full` when a granular escape hatch is needed.
6. If a client cannot call `list_tool_profiles`, it can still use the metadata fallback: keep the full `tools/list`, sort/prefer tools in the requested profile from `_meta`, and use `advanced-full` as the documented escape hatch.
7. Strict filtering must preserve the `readonly` restriction (`readOnlyHint=true` only). Profile aliases still resolve before registration filtering.

## Facade implementation status

Facade v1 now has a focused implementation for `inspect_document`, `list_tool_profiles`, `read_content`, `search_document`, `preview_edit`, `apply_edit`, `validate_content`, `load_guidance`, `manage_items`, `manage_assets`, and `manage_file`. These tools compose existing HTTP routes (except `list_tool_profiles`, which is taxonomy-only) and report routed_legacy, touched_targets, guard values/previews where applicable, and facade `next_actions`. `read_content` defaults to a 24KB response cap; root surface selectors return a structure overview plus continuation hints and recommended follow-up selectors unless the caller explicitly sets `selector.include_raw=true` and an explicit byte cap. `read_content` also covers active/reference/external lorebook, regex, greeting, trigger, Lua, CSS, and `.risup` prompt selectors where the family is supported. `preview_edit` / `apply_edit` cover active/external field write/replace, active/external surface patch, active lorebook text replacement, active lorebook id writes/deletes, active indexed or identity regex/greeting writes/deletes, external lorebook/regex/greeting structured write/delete/replace flows through guarded surface patches, active/external trigger writes/deletes, active/external Lua/CSS section write/replace/insert, external Lua/CSS section delete, active and external `.risup` prompt-item write/delete by id or index, active regex/greeting/risup prompt batch writes, active greeting/risup prompt batch deletes, and active/external `.risup` prompt-item batch writes/deletes with derived stale guards. `manage_items` covers active/external `.risup` prompt add, reorder, text import, copy-as-text, persistent snippet list/read/save/insert/delete, and active/external lorebook, regex, alternate greeting, trigger, Lua, and CSS add/reorder through read/preview/apply modes. `manage_assets` covers active/external `.charx` and `.risum` asset list/read plus preview/apply add/delete/rename and `.charx` `compress_assets` workflows with collection digest guards, `expected_path` guards for destructive item operations, and active/external document `expected_hash` guards where surface patches are used. `manage_file` covers active/session snapshot listing, project-tree reads, and preview/apply file open, active save, snapshot/restore, field export, and `.charx`/`.risum`/`.risup` project-folder extract/reassemble workflows with active-file, file-state, output-state, and project-tree guards. `validate_content` covers active lorebook, regex, active/external `.charx` export compatibility via `family: "asset"` or `field: "exportCompatibility"`, CBS, Danbooru, active/external `.risup` prompt/order checks, `.risum` semantic fields, and external Plugin v3 source scans when selectors provide enough context. Granular tools remain available as stable advanced routes for `.risum` compression, exact legacy payloads, and unsupported facade cases.
...
| `patch_surface` / `external_patch_surface` on active or external surfaces                                                                                                                                                                                      | `preview_edit` then `apply_edit` for active/external surface patch operations                                                         | Implemented for preview-token-first active and unopened-file surface patch with `expected_hash` guards.                                                                                                                                                     | You need `replace_in_surface`, cross-surface workflows, root-hash diagnostics, exact legacy response shapes, or unsupported patch shapes.                                                |
...
| Trigger, Lua, and CSS add/reorder workflows on active or external files                                                                                                                                                                                        | `manage_items` with `family="trigger"`, `family="lua"`, or `family="css"` and `mode="preview"` then `mode="apply"`                    | Implemented for active/external add and full index reorder with item collection digests. External writes also carry document `expected_hash`; active writes use field-write sync.                                                                           | You need unsupported batch shapes, active Lua/CSS section delete, exact legacy add/reorder response shapes, or raw JSON Pointer diagnostics.                                             |
| `.charx` / `.risum` asset list/read/add/delete/rename, and `.charx` asset compression on active or external files                                                                                                                                              | `manage_assets` with `asset_family="auto"`, `"charx"`, or `"risum"` and `mode="read"`, `mode="preview"`, then `mode="apply"`          | Implemented for active/external asset list/read/add/delete/rename plus `.charx` `compress_assets`. Active `.charx` add/delete/rename and active `.risum` add/delete use existing asset tools; `.risum` rename and external apply use guarded surface patch. | You need `.risum` compression, exact legacy response shapes, or raw asset/surface diagnostics.                                                                                           |
| Session/file management, snapshots, field export, and project-folder extract/reassemble                                                                                                                                                                        | `manage_file` with `mode="read"`, `mode="preview"`, then `mode="apply"`                                                               | Implemented for snapshot list, project-tree reads, guarded file open, active save, snapshot/restore, active field export, and external `.charx`/`.risum`/`.risup` project-folder extract/reassemble workflows.                                              | You need exact legacy response shapes, unsupported filesystem operations, non-artifact files, or precision file/debug workflows.                                                         |
...
3. **Unsupported facade operation:** facade edits cover active/external field write/replace, active/external surface patch, active/external lorebook text replacement or item write/delete where selectors resolve, active/external regex/greeting identity or index writes/deletes where selectors resolve, active/external trigger write/delete, active/external Lua/CSS section write/replace/insert, external Lua/CSS section delete, active/external `.risup` prompt-item id/index writes/deletes, older indexed regex/greeting/risup prompt item writes/deletes, supported prompt/greeting/regex batch shapes, `.risup` prompt item/snippet management, lorebook/regex/alternate greeting/trigger/Lua/CSS add/reorder through `manage_items`, covered `.charx`/`.risum` asset list/read/add/delete/rename and `.charx` compression through `manage_assets`, and covered file/session/project-folder workflows through `manage_file`. Block replacements, unsupported batch semantics, `.risum` compression, non-artifact filesystem operations, and non-patch surface operations stay granular.
...
- **Known gaps:** unsupported batch structured item editors, unsupported trigger/Lua/CSS batch mutations, unsupported asset operations such as `.risum` compression, CBS simulation/diff, Danbooru search, non-patch surface operations, non-artifact filesystem operations, and broad validators without enough selector context remain outside facade scope.
...
| `search_document`  | read-only  | **Implemented.** Search active documents, specified external/reference fields, or external `.risup` prompt items, excluding hidden deprecated/legacy/reserved compatibility fields.                                                                                                                                                                    |
| `preview_edit`     | preview    | **Implemented.** Produce a dry-run/read-only preview token for active/external field replace/write, active/external surface patch, external lorebook/regex/greeting structured edits, external `.risup` prompt item writes/deletes, and active indexed/batch structured item operations.                                                               |
| `apply_edit`       | mutating   | **Implemented.** Apply a prior `preview_edit` by `preview_token` + `operation_digest`, carrying explicit or preview-derived guard values forward, including external structured item stale guards and document `expected_hash`.                                                                                                                        |
| `validate_content` | read-only  | **Implemented.** Validates active lorebook, regex, active/external `.charx` export compatibility, CBS, Danbooru, active/external `.risup` promptTemplate/formatingOrder structure, active/external `.risum` semantic fields, and external Plugin v3 source scans.                                                                                      |
...
| `manage_assets`    | mutating   | **Implemented.** Read/preview/apply facade for active/external `.charx` and `.risum` asset list/read/add/delete/rename plus `.charx` `compress_assets`, with asset collection digest, `expected_path`, and active/external `expected_hash` guards where surface patches are used.                                                                      |
...
- **Boundary:** prefer `manage_assets` action `compress_assets` for covered active/external `.charx` compression; use this granular route for exact legacy payloads, compatibility testing, or unsupported direct compression workflows

### `asset-management`

- **Use when:** preferred facade for active/external `.charx` and `.risum` asset list/read/add/delete/rename workflows plus `.charx` asset compression
- **Tools:** `manage_assets`
...
- **Boundary:** keep granular asset tools for exact legacy response shapes, unsupported `.risum` compression, raw asset diagnostics, or unsupported filesystem workflows
...
- **Use when:** preferred facade for guarded file/session workflows such as opening an unopened artifact, saving the active document, listing or restoring snapshots, exporting an active field, and extracting/reassembling `.charx`, `.risum`, or `.risup` project folders
- **Tools:** `manage_file`
...
- **Guard coverage:** file open/extract uses `expected_file_state_digest`; save/snapshot/export uses `expected_active_file_path`; field export and project outputs use `expected_output_state_digest`; reassemble uses `expected_project_tree_digest`
