...
The authoring surface follows [OpenAI function-tool design guidance](https://developers.openai.com/api/docs/guides/function-calling#best-practices-for-defining-functions): expose clear inputs, reduce invalid argument combinations, keep the initially available set small, and measure task outcomes. [GPT-6 Astra guidance](https://developers.openai.com/api/docs/guides/latest-model?model=gpt-6-astra) also motivates clear authorization and verification boundaries in the authoring skills. These design choices do not establish model-specific performance gains; the local MCP tests measure server contracts and artifact outcomes.
...
- The 14 default tools expose their actual action-specific input schemas plus a compact common `outputSchema`, and return the same parsed envelope through `structuredContent` while preserving the existing text JSON. Handler-side validation remains authoritative for cross-field conditions.
- The complete default `tools/list` response is statically guarded at 96 KiB and each serialized tool definition at 24 KiB. The current measured counts and byte sizes live in `test/fixtures/mcp-module-split-contract.json`; complete required inputs take precedence over discriminator-only schema compression.
...
- Recovery metadata includes stable `code`, `retry_mode`, and `outcome`; mutation timeout, cancellation after dispatch, and partial apply require outcome inspection instead of automatic retry.
...
- High-traffic tools may return narrower per-tool `next_actions` than the family default. The MCP response boundary keeps follow-ups within the registered profile, mapping hidden routes to supported facades when available. These are optional next steps for the current task; use existing evidence when it already satisfies verification.
...
- `manage_items`, `manage_assets`, and `manage_file` additionally expose `risutoki/modes`, distinguishing side-effect-free read/preview from confirmation-required apply while keeping conservative static annotations for older clients.
...
### Bot authoring and regression checks

The default surface supports creation through `manage_file` with `mode="preview"` and `operation={action:"create_document",file_path:"/absolute/new.charx",name:"Librarian"}` on a session target. Apply the returned token, digest, and guards. The parent directory must exist; creation never overwrites an existing path or switches the active editor document. Use external reads/edits directly or explicitly open the new file.

`read_content` accepts a single field selector with `offset` and `length` (at most 10,000 UTF-16 units per source page). Offsets use LF-normalized text, matching `search_document`. Follow `next_cursor` with the same target/field and `cursor` instead of offset/length. Pages fit the complete JSON response budget, preserve Unicode characters, and reject changes to the source; cursors expire after ten minutes or a server restart. Range selectors cannot be used for edits.

Reference item reads use `index` or `indices`; omit item identity to list the family. Reference selectors containing `id`, `ids`, or `identity` return `400` with the supported `read_content` route instead of silently returning a list or ignoring the identity.

`manage_assets` add operations accept exactly one of `base64` or absolute `source_path`. Local source files are capped at 8 MiB, and `expected_source_hash` binds the preview to the source bytes. Carry every returned guard through apply.

`evaluate_bot` is a read-only, default-profile regression tool for active or external artifacts. Each of up to 30 named cases requires explicit expectations:

```json
{
  "target": { "kind": "external", "file_path": "/absolute/bot.charx" },
  "cases": [
    { "name": "character description", "kind": "field", "field": "description", "contains": ["librarian"] },
    { "name": "output transformation", "kind": "regex", "mode": "editoutput", "text": "hello", "equals": "hello" },
    {
      "name": "library activation",
      "kind": "lorebook",
...
Reuse the same cases before and after an edit. `passed`, pass/fail counts, per-case checks and the source fingerprint describe deterministic expectations, not LLM roleplay quality. A changed source rejects the run. Bounded reports preserve aggregate outcomes and prioritize failed cases; rerun a smaller case subset when details are omitted. Lorebook simulation does not apply a model token budget, and the tool does not execute Lua, render HTML/CSS, call a model, or assemble a provider request. Continue to use `validate_content` for syntax and export compatibility.

### Existing facade coverage

Guidance target `{ kind: "guidance" }` lists skills plus the guide-document names under the `risu/*/docs` roots; adding `skill` and optional `document` reads a skill document, including the `references/<name>.md` files that `list_skills` reports in `files`; adding `guide` instead reads one guide document by catalog name (`prompts/families/PHEME.md`), repository path (`risu/prompts/docs/families/PHEME.md`), or unique file name, with the same byte bounding and resume cursor as `read_skill`. `search_document` uses the same field/risup-prompt selector union as `read_content`; the old `field` argument and `field="risup-prompt"` magic value remain deprecated aliases. `preview_edit` uses operation-specific schemas: `replace_text` requires `find`, and write/insert/patch operations require `content`.

Facade v1 provides `inspect_document`, `list_tool_profiles`, `read_content`, `search_document`, `analyze_content`, `evaluate_bot`, `preview_edit`, `apply_edit`, `validate_content`, `load_guidance`, `manage_items`, `manage_assets`, and `manage_file`; `load_guidance` is retained as legacy compatibility rather than default registration. Default artifact facade results use a 24KB UTF-8 result bound with a 64KB hard maximum; Skill bootstrap uses a compatibility-oriented 64KB maximum. Semantic truncation keeps the envelope, preview token, digest, guards, target, and evidence parseable while reporting original/returned sizes, omitted counts, and narrower-read guidance. `analyze_content` owns transformation/statistics/simulation: `field_stats`, explicit-encoding `token_count`, `simulate_lorebook`, `test_regex`, CBS/Danbooru analysis, diffs, and `verify_risup_prompt_import` (`validate_risup_prompt_import` remains an alias). `validate_content` owns pass/fail diagnostics, including compile-only Lua section and `triggerlua` syntax checks plus Danbooru `valid | invalid | unknown` results. `preview_edit` / `apply_edit` cover Lua/CSS section deletion and consume preview tokens before the first mutation; batch failures report applied operations, the failed operation, remaining count, and require a fresh inspection/preview. `manage_assets` supports active/external `.charx` and `.risum` WebP compression, updating `.risum` binary/extension metadata together while preserving failed or larger conversions.
...
Stage 1 entered soft legacy on **2026-07-03** after `npm run test:evals:replay` completed **12/12 scripted scenarios** covering mappings for **35/35 replayable catalog tasks** in **29,521 ms**. The deterministic server-contract ratios were all **1.0**, with `wrongTargetIncidents` at **0**. These numbers describe scripted MCP conformance, not model tool-selection accuracy.
...
`test/workflow-eval-catalog.ts` is the 39-task declarative workflow-contract matrix. `src/lib/mcp-agent-workflow-eval.test.ts` validates its route, safety, classification, catalog-scenario mapping, local-corpus, and documentation invariants without claiming model or runtime success from literals.

`npm run test:evals:replay` executes 12 scripted scenarios against synthetic artifacts through `tools/list` and `callTool`. It reports `scenarioCompletion`, `requiredRouteConformance`, `firstPassCompletion` for scenarios without designed rejection/recovery, actual response-target mismatches, validation coverage, bounded-read coverage, call budgets, catalog-scenario coverage, and independent final artifact state. Expected stale-guard and missing-preview recovery attempts are counted separately. The runner does not execute task prompts or call a language model.
...
First-wave requests are bounded by contract: at most 50 selectors/operations/guard entries, at most 100 search matches, and a 65536 byte maximum. Truncated results remain valid structured objects and expose `truncated`, `omitted_count`, original/returned byte sizes, and a continuation hint; callers follow that hint with a narrower selector, search, or range read rather than parsing an incomplete JSON prefix.
