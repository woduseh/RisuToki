...
| `write_regex`, `delete_regex`, `write_greeting`, `delete_greeting`, `write_risup_prompt_item`, `delete_risup_prompt_item` for one active indexed item | `preview_edit` then `apply_edit` with `selector.family` + `selector.index`    | Implemented for active indexed write/delete operations with derived stale guards.                  | You need batch writes/deletes, add/reorder, import/export/snippet workflows, or exact legacy response shapes.                                                                                  |
...
3. **Unsupported facade operation:** facade edits cover active/external field write/replace, active surface patch, active lorebook text replacement, and active indexed regex/greeting/risup prompt item writes/deletes. Inserts, block replacements, batch item writes/deletes, add/reorder operations, asset compression, external surface patches, and item/file management stay granular.
...
- **Known gaps:** `validate_content` covers active lorebook key validation plus active `.risup` promptTemplate/formatingOrder structure checks, `load_guidance` covers skill catalog/document reads only, and item management (`manage_items`), asset management (`manage_assets`), file management (`manage_file`), batch structured item editors, add/reorder operations, other structured delete families, imports/exports/snippets, external surface patches, CBS/Danbooru validation, and broad batch validation remain outside facade scope.
- **Post-v0.69.3 follow-up backlog:** extend mutating facade parity beyond single indexed regex/greeting/risup prompt item writes/deletes into batch/add/reorder workflows where guard alignment is clear, and broaden `validate_content` only where facade selectors have clear validator parity. Until then, keep using granular validators and structured mutation families for unsupported cases.
