...
| `preview_edit`     | preview    | **Implemented.** Produce a dry-run/read-only preview token for active/external field replace/write, active/external surface patch/replace, external lorebook/regex/greeting structured edits, external `.risup` prompt item writes/deletes, and active indexed/batch structured item operations.                                                       |
