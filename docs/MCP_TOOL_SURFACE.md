...
- The 13 default tools expose compact public input discriminators plus a compact common `outputSchema`, and return the same parsed envelope through `structuredContent` while preserving the existing text JSON. The original detailed Zod shape is revalidated immediately before the existing handler, so schema compression does not loosen accepted payloads.
- The complete default `tools/list` response is statically guarded at 42 KiB and each serialized tool definition at 10 KiB. The 1.14.0 baseline is 29,484 bytes with a largest single definition of 2,958 bytes.
