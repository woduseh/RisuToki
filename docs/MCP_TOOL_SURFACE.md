...
| --------------- | ------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `facade-first`  | Yes     | Implemented facade/catalog tools: `inspect_document`, `list_tool_profiles`, `read_content`, `search_document`, `preview_edit`, `apply_edit`, `validate_content`, `load_guidance`, `manage_items`, `manage_assets` | In default compatibility mode, `tools/list` remains unfiltered; in strict mode, only facade-first tools are registered.          | Use `advanced-full` when the facade cannot express the target, selector, operation, bounds, response detail, or compatibility requirement. |
...
| `readonly`      | No      | Only tools with `readOnlyHint=true`, including the read-only first-wave facade tools                                                                                                                              | `preview_edit`, `apply_edit`, `manage_items`, `manage_assets`, every mutating/destructive tool, and session/file-open mutations. | Request `advanced-full` only after leaving read-only mode intentionally.                                                                   |
...
| `.charx` / `.risum` asset list/read/add/delete and `.charx` asset rename on active or external files                                                                                                                                                           | `manage_assets` with `asset_family="auto"`, `"charx"`, or `"risum"` and `mode="read"`, `mode="preview"`, then `mode="apply"`          | Implemented for active/external asset list/read/add/delete plus `.charx` rename. Active apply routes through existing asset tools; external apply uses guarded surface patch.     | You need asset compression, unsupported `.risum` rename, exact legacy response shapes, or raw asset/surface diagnostics.                                                                 |
...
- **Implemented parity:** first-wave read/search/preview/apply routes plus the `manage_items` and `manage_assets` facades compose existing HTTP routes where possible and report routed_legacy, touched_targets, guard values/previews, facade bounds/truncation metadata, and facade `next_actions`.
- **Known gaps:** file management (`manage_file`), unsupported batch structured item editors, unsupported trigger/Lua/CSS batch mutations, unsupported asset operations such as compression or `.risum` rename, CBS simulation/diff, Danbooru search, non-patch surface operations, and broad validators without enough selector context remain outside facade scope.
- **Follow-up backlog:** extend mutating facade parity into file workflows and remaining structured families only when preview can show exact insertion/order/file effects and stale guard policy is unambiguous. Until then, keep using granular validators and structured mutation families for unsupported cases.
...
| `validate_content` | read-only  | **Implemented.** Validates active lorebook, regex, `.charx` export compatibility, CBS, Danbooru, active/external `.risup` promptTemplate/formatingOrder structure, active `.risum` semantic fields, and external Plugin v3 source scans.                                                                                                               |
...
| `manage_assets`    | mutating   | **Implemented.** Read/preview/apply facade for active/external `.charx` and `.risum` asset list/read/add/delete plus `.charx` rename, with asset collection digest, `expected_path`, and external `expected_hash` guards.                                                                                                                              |

Future facade candidates are documented but not part of first-wave v1: `manage_file`.
...
1. `preview_edit` accepts bounded `operations[]`, while `manage_items` accepts one item-management operation for `.risup` prompts/snippets or lorebook/regex/alternate greeting/trigger/Lua/CSS add/reorder and `manage_assets` accepts one asset-management operation. These preview tools return a `facade-preview-v1.*` `preview_token`, an `operation_digest`, and `required_guards`.
2. `apply_edit`, `manage_items`, or `manage_assets` apply mode requires the preview token, operation digest, target, and current guard values.
