...
루트 `skills/` 폴더는 프로젝트 스킬 문서를 둘 수 있는 선택적 위치입니다. 현재 저장소에는 번들 스킬 문서가 포함되지 않아 이 폴더가 비어 있을 수 있습니다.
...
Windows에서 git 체크아웃이 링크를 일반 텍스트 파일(`../skills`)로 풀어버리면 `npm run sync:skills`를 실행하세요. 이 명령은 `npm install`의 `prepare` 단계에서도 자동 실행되며, 루트 `skills/` 폴더가 없으면 조용히 건너뜁니다.
