...
| 🔧 **MCP 도구 50+종** | 필드·로어북·정규식·Lua/CSS 섹션·인사말·트리거·에셋·참고자료·Danbooru 태그·스킬 문서 읽기/쓰기 + 일괄 검색/치환 |
...
- GitHub Actions `CI` — push/PR 시 lint · typecheck · test · build 자동 검증
...
| **Lua 섹션** | `list_lua` · `read_lua` · `write_lua` · `replace_in_lua` · `insert_in_lua`                                                                                                      |
| **CSS 섹션** | `list_css` · `read_css` · `write_css` · `replace_in_css` · `insert_in_css`                                                                                                      |
...
| **정규식**   | `list_regex` · `read_regex` · `write_regex` · `add_regex` · `delete_regex` · `replace_in_regex` · `insert_in_regex`                                                             |
...
| **참고자료** | `list_references` · `read_reference_field` + 로어북/Lua/CSS 세부 조회 도구                                                                                                      |
| **에셋**     | charx: `list_charx_assets` · `read_charx_asset` · `add_charx_asset` · `delete_charx_asset` · `rename_charx_asset` / risum: `list_risum_assets` 등                               |
