...
[![Version](https://img.shields.io/badge/version-0.26.0-blue.svg)](https://github.com/woduseh/RisuToki/releases)
...
| 🔧 **MCP 도구 114종** | 필드·로어북·정규식·Lua/CSS 섹션·인사말·트리거·risup promptTemplate/formatingOrder·에셋·CBS 검증·참고자료·Danbooru 태그·스킬 문서 읽기/쓰기 + 일괄 검색/치환 |
...
- `promptTemplate`는 raw JSON textarea 대신 **카드형 항목 목록 + 상세 편집기**로 열리며, `type`, `type2`, `role`, `text`, `range`, `innerFormat`, `defaultText`, cache 옵션을 구조적으로 수정할 수 있습니다.
- `formatingOrder`는 raw JSON textarea 대신 **재정렬 가능한 토큰 목록**으로 열리며, known/unknown token을 그대로 유지합니다.
...
| GitHub Copilot CLI | `~/.copilot/mcp-config.json` | `AGENTS.md` 자동 생성 + 프로젝트 가이드 병합 + 플루니 모드 시 `.github/agents/*.md` 임시 생성 |
...
- `플루니 연구소` RP 모드에서 GitHub Copilot CLI를 시작하면 선택한 카테고리에 맞춰 `Pluni` / `Kotone` / `Sophia` custom agent 프로필이 `.github/agents/` 아래에 임시 생성됩니다.
- GitHub Copilot CLI가 아닌 환경에서는 같은 3인 자문 구조를 단일 세션 프롬프트로 합성해 전달합니다.

### 프로젝트 스킬 폴더

프로젝트 스킬의 단일 편집 지점은 루트 `skills/` 폴더입니다.
...
Windows에서 git 체크아웃이 링크를 일반 텍스트 파일(`../skills`)로 풀어버리면 `npm run sync:skills`를 실행하세요. 이 명령은 `npm install`의 `prepare` 단계에서도 자동 실행됩니다.
...
- 로어북 폴더는 폴더 항목의 `key` UUID를 기준으로 추적되며, 자식 항목의 `folder` 값은 항상 `folder:UUID` 형태로 정규화됩니다. 예전 `id` 기반 폴더 데이터도 읽기 시 자동 호환됩니다.
...
- GitHub Copilot CLI에서는 `Pluni` / `Kotone` / `Sophia` 임시 자문 에이전트를 생성하고, 다른 CLI에서는 같은 자문 구조를 단일 세션 프롬프트로 합성합니다.
...
- GitHub Copilot CLI 시작 시 세션 `AGENTS.md`와 임시 `.github/agents/pluni.md`, `kotone.md`, `sophia.md`를 생성합니다.
...
- MCP에서는 raw `read_field` / `write_field` 외에도 prompt item CRUD / reorder와 formatting order 전용 도구를 제공합니다.
