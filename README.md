...
| 📚 **Movable Managers**    | RisuMari-style `.charx` lorebook and asset panels with search, filters, thumbnail browsing, multi-select, bulk delete, and normal layout drag-and-drop placement                                                                                                                                                                                                                                               |
...
- `.charx` and `.risum` files also show independent **로어북 관리자** and **에셋 관리자** panels. Drag their headers to place them like other panels, close them into edge expand buttons, use lorebook search/filter/folder browsing, and use asset thumbnail browsing, drag-drop image adds, multi-select, and bulk deletion. `.risup` presets hide these managers because they do not use the same lorebook/asset workflow.
- MCP agents can also use project-folder tools when a field or whole document is too large to handle comfortably through MCP responses.
