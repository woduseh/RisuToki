...
[![Version](https://img.shields.io/badge/version-0.72.14-blue.svg)](https://github.com/woduseh/RisuToki/releases)
...
- Facade-first tools (`inspect_document`, `read_content`, `search_document`, `preview_edit`, `apply_edit`, `validate_content`) cover bounded read/search plus preview-token-first active/external field edits, active surface patches, active lorebook text/id edits, regex/greeting identity edits, and `.risup` prompt-item id edits. LLM workflows should prefer `{ family: "risup-prompt", id }`, `{ family: "lorebook", id }`, and regex/greeting `identity` selectors before falling back to index-based tools.
