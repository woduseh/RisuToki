...
```
┌─────────────────────────────────────────────────┐
│ 파일  편집  보기  터미널  설정          파일명    │ ← 메뉴바
├──────────┬──────────────────────────────────────┤
│          │ [탭1] [탭2] [탭3]              [↗][×]│ ← 탭 바
│ 사이드바 │                                      │
│ (항목/   │         Monaco 에디터                 │ ← 에디터 영역
│ 참고자료)│                                      │
│          │                                      │
├──────────┴──────────────────────────────────────┤
│ 🐰 아바타 │ 💬 TokiTalk  [🐰][🔇][🖼][⚙][━]   │ ← 터미널 영역
...
터미널 출력을 메신저 스타일의 채팅 버블로 표시하는 모드입니다.

### 사용법
1. 터미널 헤더의 💭 버튼 클릭으로 토글
2. Claude Code 실행 중 활성화하면 응답이 채팅 버블로 표시됩니다
3. 선택지가 포함된 응답(`1. xxx / 2. yyy`)은 클릭 가능한 버튼으로 렌더링됩니다

### 특징
- 터미널 ↔ 채팅 모드 자유롭게 전환 가능
- 전환 시 기존 응답이 유지됩니다
- 채팅 모드에서도 하단 입력창으로 메시지 전송 가능
