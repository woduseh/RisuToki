...
| Category             | Tools                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Facade v1**        | `list_tool_profiles` · `inspect_document` · `read_content` · `search_document` · `preview_edit` · `apply_edit` · `validate_content` · `load_guidance` · `manage_items` · `manage_assets` — bounded/profiled workflow entrypoints, including external lorebook/regex/greeting/trigger/Lua/CSS and `.risup` prompt item read/preview/apply/validate, active/external surface patch preview/apply, `.risup` prompt item/snippet management, lorebook/regex/alternate greeting/trigger/Lua/CSS add/reorder, `.charx`/`.risum` asset list/read/add/delete plus `.charx` rename, and runtime health metadata |
...
| **Assets**           | facade: `manage_assets` for covered active/external `.charx`/`.risum` asset list/read/add/delete and `.charx` rename / granular: `list_charx_assets` · `read_charx_asset` · `add_charx_asset` · `delete_charx_asset` · `rename_charx_asset` · `list_risum_assets` · `read_risum_asset` · `add_risum_asset` · `delete_risum_asset` · `compress_assets_webp`                                                                                                                                                                                                                                             |
...
- Facade-first tools (`inspect_document`, `read_content`, `search_document`, `preview_edit`, `apply_edit`, `validate_content`, `manage_items`, `manage_assets`) cover bounded read/search plus preview-token-first active/external field edits, active/external surface patches, active/external lorebook text/id edits, regex/greeting identity or index edits, trigger writes/deletes, Lua/CSS section writes/replacements/inserts, `.risup` prompt-item id/index edits, active/external `.risup` prompt add/reorder/import/copy/snippet workflows, active/external lorebook, regex, alternate greeting, trigger, Lua, or CSS add/reorder, and covered `.charx`/`.risum` asset management. LLM workflows should prefer `{ family: "risup-prompt", id }`, `{ family: "lorebook", id }`, regex/greeting `identity` selectors, trigger/Lua/CSS section selectors, `manage_items`, and `manage_assets` before falling back to index-based or granular tools.
- For unopened files, the recommended flow is facade `inspect_document` / `read_content` first, then `preview_edit` / `apply_edit` for covered field, surface, lorebook, regex, alternate greeting, trigger, Lua/CSS section, or `.risup` prompt-item mutations; use `manage_assets` for covered external `.charx`/`.risum` asset management; use `probe_*` / `external_*` for exact legacy shapes or unsupported direct path edits, and `open_file` only when active-document tools are needed.
...
- Via MCP, the preferred `manage_assets` facade handles active/external `.risum` asset list/read/add/delete through preview-first workflows with collection and stale-path guards; use granular asset routes for exact legacy payloads or unsupported asset operations.
