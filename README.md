...
[![Version](https://img.shields.io/badge/version-0.6.0-blue.svg)](https://github.com/komodoD/RisuToki/releases)
...
RisuToki는 [RisuAI](https://risuai.net/) 캐릭터 카드(`.charx`), 모듈(`.risum`), 프리셋(`.risup`) 파일을 편집하기 위한 **데스크톱 전용 에디터**입니다. VS Code 수준의 Monaco 에디터와 내장 터미널을 통해 AI CLI(Claude Code, GitHub Copilot CLI, Codex)를 직접 연동할 수 있으며, MCP(Model Context Protocol)로 파일 구조를 AI에 자동 전달합니다.
...
| 🤖 **AI CLI 연동**    | Claude Code · GitHub Copilot CLI · Codex 터미널 내 실행 + MCP 자동 연결         |
| 📦 **3종 파일 지원**  | `.charx` (캐릭터 카드) · `.risum` (모듈) · `.risup` (프리셋) 열기/편집/저장     |
| 🔧 **MCP 도구**       | 필드·로어북·정규식·Lua 섹션·CSS 섹션 읽기/쓰기 — AI가 에디터 데이터에 직접 접근 |
...
.charx 파일을 열면 다음 항목들이 표시됩니다 (.risum, .risup 파일은 해당 파일 타입에 맞는 항목만 표시):
...
| 항목                | 설명                                     | 편집 언어   | 파일 타입           |
| ------------------- | ---------------------------------------- | ----------- | ------------------- |
| **Lua**             | Lua 트리거 스크립트                      | Lua         | charx, risum        |
| **트리거 스크립트** | 전체 트리거 스크립트 JSON                | JSON        | charx, risum        |
| **글로벌노트**      | 시스템 프롬프트 뒤에 삽입되는 인스트럭션 | 일반 텍스트 | charx               |
| **첫 메시지**       | 대화 시작 시 표시되는 첫 번째 메시지     | HTML        | charx               |
| **추가 첫 메시지**  | 대체 첫 메시지 배열                      | JSON        | charx               |
| **그룹 첫 메시지**  | 그룹 전용 인사말 배열                    | JSON        | charx               |
| **CSS**             | RisuAI 채팅 UI에 적용되는 커스텀 스타일  | CSS         | charx               |
| **기본변수**        | 기본 변수 값                             | 일반 텍스트 | charx               |
| **설명**            | 캐릭터/모듈/프리셋 설명                  | 일반 텍스트 | charx, risum, risup |
| **메인 프롬프트**   | AI 프리셋 기본 프롬프트                  | 일반 텍스트 | risup               |
| **제일브레이크**    | AI 프리셋 제일브레이크 프롬프트          | 일반 텍스트 | risup               |
...
- **Gemini 시작**: 현재 파일 컨텍스트와 함께 Gemini CLI 실행
- **터미널 지우기**: 화면 초기화
- **터미널 재시작**: 셸 프로세스 재시작
...
RisuToki의 핵심 기능입니다. 터미널에서 Claude Code, GitHub Copilot CLI, Codex를 실행하면 열려 있는 .charx / .risum / .risup 파일의 구조와 내용을 자동으로 전달합니다.
...
| Gemini CLI         | 터미널 메뉴 → **Gemini 시작**             | `AGENTS.md` 자동 생성 + 프로젝트 가이드 병합   | `~/.gemini/settings.json`    |

### 시작 방법

1. .charx / .risum / .risup 파일을 연 상태에서
...
| `list_lua` / `read_lua` / `write_lua`                | Lua 섹션 조회/읽기/수정           |
| `replace_in_lua` / `insert_in_lua`                   | Lua 섹션 부분 치환/삽입           |
| `list_css` / `read_css` / `write_css`                | CSS 섹션 조회/읽기/수정           |
| `replace_in_css` / `insert_in_css`                   | CSS 섹션 부분 치환/삽입           |
...
- Codex, GitHub Copilot CLI, Gemini CLI는 세션 동안 사용할 `AGENTS.md`를 자동 생성하며, 기존 `AGENTS.md`가 있으면 그 프로젝트 가이드를 세션 컨텍스트와 함께 병합합니다. 작업 폴더에 가이드가 없더라도 앱에 내장된 `CLAUDE.md` 가이드를 자동으로 주입한 뒤, 종료 시 원래 파일을 복원하거나 임시 파일을 정리합니다.
...
- .charx / .risum 파일은 메인 파일 없이도 참고 자료로 추가 가능
