...
| 🎭 **Preview Mode**         | Persistent `.charx` local chat simulation beside the editor, explicit current-draft refresh, greeting/viewport/debug state preservation, and source links; sanitized rendered Markdown previews (F5)                                                                                                                                                                                                           |
| 🔍 **Change Review**        | Compare the unsaved structured document and assets with the saved file/project, detect external changes, and restore supported changes into the draft before saving                                                                                                                                                                                                                                            |
...
| ⬆️ **Update Notifications** | Checks packaged builds for each new stable release once; installer users can approve an in-app download that installs on normal exit, while portable users can open the latest GitHub release page                                                                                                                                                                                                             |
...
Packaged builds check for each newer stable release once. Installer builds ask before downloading and install an accepted update when the app is normally closed. Portable builds only offer to open the latest GitHub release page.
...
> **Facade migration note:** 3.6.0 uses 14 bootstrap-capable tools: 12 preferred facades plus `list_skills` and `read_skill`. App-managed connections migrate broad profiles to `facade-first`, preserving `readonly`. Headless defaults to the same surface. Explicit `--tool-profile advanced-full` remains a compatibility option; restart existing MCP processes after configuration changes.
...
| **Shared Presentation** | `writing-restricted-wysiwyg-html`                                                                                                                                    | Target-aware restricted WYSIWYG HTML with conditional platform profiles     |
| **Asset Prompting**     | `writing-standing-image-prompts`                                                                                                                                     | Target-aware standing-image prompt composition and Danbooru tag validation  |
| **Bot Authoring**       | `authoring-bots`, `writing-translation-guides`                                                                                                                       | Project-specific bot formats, artifact placement, and translation contracts |
| **Preset Authoring**    | `writing-risup-presets`, `prompt-family`                                                                                                                             | Preset structure and maintained Phēmē design/synchronization                |
...
When an AI CLI starts, the MCP server connects automatically so the AI can read and write the active document directly. By default `tools/list` contains 14 `facade-first` tools with complete action-specific inputs, a common `outputSchema`, and equivalent JSON text and `structuredContent`. Detailed validation also runs before execution. The schema budget is 96 KiB overall and 24 KiB per tool; measured sizes are recorded in `test/fixtures/mcp-module-split-contract.json`. `advanced-full` retains 204 compatible facade and granular tools. Normal bot authoring can create artifacts, read long fields in guarded pages, edit content, import assets by file path, and run explicit regression cases through the default surface.

| Category             | Tools                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Facade v1**        | default: `list_tool_profiles` · `inspect_document` · `read_content` · `search_document` · `analyze_content` · `evaluate_bot` · `preview_edit` · `apply_edit` · `validate_content` · `manage_items` · `manage_assets` · `manage_file`; legacy compatibility: `load_guidance` — byte-bounded/profiled workflow entrypoints, including field/token statistics, lorebook/regex simulation, Lua validation, guarded edits, asset management, and file/session/project-folder management |
