...
import { parsePromptTemplate } from './src/lib/risup-prompt-model';
import {
...
const { previewManageAssetsOperation, readManageAssetsOperation, applyManageAssetsOperation } =
  createFacadeAssetsEngine({
    apiRequest,
    hashStableValue,
    readExternalSurfaceValue,
  });
const facadeItemsEngine = createFacadeItemsEngine(apiRequest, facadeScriptStyleEngine);
const { previewManageItemsOperation, readManageItemsOperation, applyManageItemsOperation } = facadeItemsEngine;
const {
  ...
  danbooru: {
    ...
    formatTags,
    getDanbooruStatus: () => {
      const filePath = path.join(__dirname, 'resources', 'Danbooru Tag.txt');
      return {
        loaded: tagsLoaded,
...
const { readActiveLorebookCollection, previewFacadeOperation, applyFacadeOperation } = facadeEditEngine;
const { readManageFileOperation, previewManageFileOperation, applyManageFileOperation } = createFacadeFilesEngine({
  apiRequest,
...
server.tool('list_lua', MCP_TOOL_DESCRIPTIONS['list_lua'], {}, async () => textResult(await apiRequest('GET', '/lua')));
