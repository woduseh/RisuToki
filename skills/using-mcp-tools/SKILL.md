...
- Deprecated `.charx` fields, `.risup` legacy prompt fields, reserved `.risum` `cjs`, and `groupOnlyGreetings` are hidden from normal MCP reads/searches/probes/surface reads and removed on save. Use `hiddenFieldWarnings` in inspect/list/status responses only as a value-safe existence summary; do not try to recover the hidden content through granular routes. `.risum` `mcpUrl` is preserved but read-only in normal mutation/string-edit routes. `.risum` `lowLevelAccess` remains visible/editable.
- For `.charx` files headed to RisuAI upload, run export compatibility validation with facade `validate_content` using an active selector such as `family: "asset"` or `field: "exportCompatibility"`. It verifies card/module lorebook and regex mirrors, canonical regex `in` / `out`, empty compatibility-only fields, card asset references, and 0-byte ZIP assets.
...
- Facade v1 is additive and preferred where implemented: use `inspect_document`, `list_tool_profiles`, `read_content`, `search_document`, `preview_edit`, `apply_edit`, `validate_content`, and `load_guidance` for bounded facade/catalog workflows; keep granular tools as advanced/legacy routes for precision or unsupported cases. `preview_edit` / `apply_edit` cover active/external field write/replace, active surface patch, active lorebook text replacement, active lorebook id writes/deletes, active regex/greeting identity writes/deletes, active `.risup` prompt-item id writes/deletes, older indexed regex/greeting/risup prompt item writes/deletes, active regex/greeting/risup prompt batch writes, active greeting/risup prompt batch deletes, and active `.risup` prompt-item id batch writes/deletes with derived stale guards. `validate_content` currently covers active lorebook key validation, regex compile/sanity checks, `.charx` export compatibility, CBS validation, Danbooru tag validation, active `.risup` promptTemplate/formatingOrder structure checks, active `.risum` semantic field consistency, and external Plugin v3 source scans. `load_guidance` covers skill catalog/document reads including Plugin v3 guidance, and item/asset/file management remain future facade work. Facade mutating flows are preview-token-first (`preview_edit` before `apply_edit`) and must propagate stale guards from granular `risutoki/staleGuardDetails`. The measurable real-artifact routing matrix lives in `src/lib/mcp-agent-workflow-eval.test.ts`.
...
| Active/external field write/replace, active surface patch, active lorebook text/id writes/deletes, active regex/greeting identity writes/deletes, active `.risup` prompt item id writes/deletes, older indexed regex/greeting/risup prompt item write/delete, covered active regex/greeting/risup prompt batch writes, covered active greeting deletes, or covered `.risup` prompt id batch writes/deletes | `preview_edit` then `apply_edit` | You need inserts, block replacements, unsupported batch semantics, add/reorder, snapshots, external surface patches, asset operations, item management, or unsupported patch semantics. |
...
- **`.risup` prompt edits:** use `list_risup_prompt_items` to get stable item ids, then facade `read_content`, `search_in_risup_prompt_items`, `read_risup_formating_order`, and `diff_risup_prompt` before edits. Prefer facade `preview_edit` → `apply_edit` with `{ family: "risup-prompt", id }` or id batches; use `reorder_risup_prompt_items_by_id` for stable reorder, and granular import/snippet/add tools for unsupported workflows. Then run `validate_content`, `validate_risup_prompt_import` after imports, and targeted by-id prompt/order re-reads.
