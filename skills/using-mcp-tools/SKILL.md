...
- Use granular tools only as advanced/legacy fallbacks for unsupported facade selectors/operations, exact structured editors, direct external mutations not covered by facade, large exact exports, unsupported `.risum` compression or unsupported asset workflows, unsupported batch workflows, or compatibility/debugging. Stable by-id/by-identity granular tools are preferred fallbacks when facade cannot express the operation.
...
- For risup prompt editing, prefer facade `read_content` / `search_document` for covered reads and `preview_edit` → `apply_edit` for covered writes/deletes. Use `manage_items` for active/external prompt add, reorder, copy-as-text, text import append/replace, and snippet list/read/save/insert/delete workflows. Use granular `list_risup_prompt_items`, `search_in_risup_prompt_items`, `read_risup_prompt_item_batch`, `export_risup_prompt_to_text`, `diff_risup_prompt`, and `read_risup_formating_order` when exact legacy shapes, full serializer output, or prompt-specific comparisons are required.
- For active/external `.charx` or `.risum` assets, use `manage_assets` for covered list/read/add/delete/rename workflows plus `.charx` compression. Use granular asset tools for unsupported `.risum` compression, exact legacy response shapes, or low-level asset diagnostics.
- For unopened files, start with facade `inspect_document` / `read_content` when covered; use `preview_edit` → `apply_edit` for covered external field/surface patch or replace, lorebook, regex, alternate greeting, trigger, Lua/CSS section, and `.risup` prompt item mutations; use `manage_assets` for covered external asset management and `manage_file` for guarded open/extract/reassemble workflows. Use `inspect_external_file` + the relevant `probe_*` reader for probe-specific summaries, then use `external_search_in_field` / `external_read_field_range` / `external_write_field*` only when you need granular result shapes or direct external mutations.
- When facade selectors and specialized tools cannot reach the required content, use the surface fallback: `list_surfaces` → `read_surface` → `patch_surface` or `replace_in_surface`. Prefer facade `preview_edit` → `apply_edit` for covered patch/replace workflows; otherwise use `dry_run` and carry the document-level `expected_hash` for risky edits.
...
- Call `list_tool_profiles` when you need a compact profile catalog: default `facade-first`, `authoring`, `readonly`, or `advanced-full` (aliases `advanced` / `full`) for the complete granular escape hatch. `tools/list` stays unfiltered for MCP compatibility unless the server was started with `--tool-profile` or `RISUTOKI_MCP_TOOL_PROFILE`, in which case only that profile's tools are registered and catalog counts report registered/hidden tools.
...
- Facade v1 is additive and preferred where implemented: use `inspect_document`, `list_tool_profiles`, `read_content`, `search_document`, `preview_edit`, `apply_edit`, `validate_content`, `load_guidance`, `manage_items`, `manage_assets`, and `manage_file` for bounded facade/catalog workflows; keep granular tools as advanced/legacy routes for precision or unsupported cases. `read_content` defaults to a 24KB cap and returns root surface overviews unless `selector.include_raw=true` is paired with explicit `max_bytes`. `preview_edit` / `apply_edit` cover active/external field write/replace, active/external surface patch/replace, active/external lorebook text replacement or item write/delete where selectors resolve, active/external regex/greeting identity or index write/delete where selectors resolve, active/external trigger write/delete, active/external Lua/CSS section write/replace/insert, external Lua/CSS section delete, active/external `.risup` prompt-item id/index writes/deletes, older indexed regex/greeting/risup prompt item writes/deletes, active/external `.risup` prompt batch writes/deletes, active regex/greeting prompt batch writes, and active greeting batch deletes with derived stale guards. `manage_items` covers active/external `.risup` prompt add, reorder, copy-as-text, text import append/replace, snippet list/read/save/insert/delete, and lorebook/regex/alternate greeting/trigger/Lua/CSS add/reorder workflows through read/preview/apply modes. `manage_assets` covers active/external `.charx` and `.risum` asset list/read/add/delete/rename plus `.charx` compression through read/preview/apply modes. `manage_file` covers active/session snapshot list, project-tree read, guarded file open, active save, snapshot/restore, field export, and `.charx`/`.risum`/`.risup` project-folder extract/reassemble workflows through read/preview/apply modes. `validate_content` currently covers active lorebook key validation, regex compile/sanity checks, active/external `.charx` export compatibility, CBS validation, Danbooru tag validation, active/external `.risup` promptTemplate checks, active/external `.risum` semantic field consistency, and external Plugin v3 source scans. `load_guidance` covers skill catalog/document reads including Plugin v3 guidance, while `.risum` compression and block-specific or cross-surface replacement workflows remain future facade work. Facade mutating flows are preview-token-first and must propagate stale guards from granular `risutoki/staleGuardDetails`. The measurable real-artifact routing matrix lives in `src/lib/mcp-agent-workflow-eval.test.ts`.
- After using `manage_items` or `import_risup_prompt_from_text` for prompt text imports, call `validate_risup_prompt_import` with the same source text to verify all items were imported correctly. This catches silent mismatches from ID renormalization and content truncation.
...
| Active/external field write/replace, active/external surface patch/replace, active/external lorebook text/id writes/deletes/replacements, active/external regex/greeting identity or index writes/deletes, active/external trigger writes/deletes, active/external Lua/CSS section writes/replacements/inserts, external Lua/CSS section deletes, active/external `.risup` prompt item id/index writes/deletes, older indexed regex/greeting/risup prompt item write/delete, covered active regex/greeting/risup prompt batch writes, covered active greeting deletes, or covered active/external `.risup` prompt batch writes/deletes | `preview_edit` then `apply_edit` | You need block replacements, unsupported batch semantics, add/reorder, snapshots, unsupported `.risum` asset compression, item management, cross-surface workflows, or unsupported patch semantics.                |
...
| `.charx` / `.risum` asset list/read/add/delete/rename, and `.charx` compression on active or unopened external files                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `manage_assets`                  | You need unsupported `.risum` compression, exact legacy response shapes, or raw asset/surface diagnostics.                                                                                                         |
| File/session/project-folder workflows: guarded open/save, snapshots, active field export, and `.charx`/`.risum`/`.risup` extract/reassemble                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            | `manage_file`                    | You need exact legacy file/session payloads, non-artifact filesystem work, or precision file/debug operations.                                                                                                     |
...
- **`.risup` prompt edits:** use `list_risup_prompt_items` to get active stable item ids, then facade `read_content`, `search_document` with `field="risup-prompt"` for external files, `search_in_risup_prompt_items`, `read_risup_formating_order`, and `diff_risup_prompt` before edits. Prefer facade `preview_edit` → `apply_edit` with `{ family: "risup-prompt", id }`, index selectors for unopened external presets, or id/index batches for writes/deletes; use `manage_items` for add, reorder, copy-as-text, import, and snippet workflows. Then run `validate_content`, `validate_risup_prompt_import` after imports, and targeted by-id/index prompt/order re-reads.
- **Asset edits:** inspect first, then use `manage_assets` read mode to list or read active/external `.charx` or `.risum` assets. For covered add/delete/rename or `.charx` compression, use `manage_assets` preview → apply and carry `expected_asset_collection_digest`, `expected_path` where applicable, and active/external `expected_hash` guards where surface patches are used. Use granular asset tools for unsupported `.risum` compression or exact legacy payloads.
- **File/session workflows:** use `manage_file` read mode for snapshot lists or project trees. For guarded file open, active save, snapshot/restore, active field export, or `.charx`/`.risum`/`.risup` project-folder extract/reassemble, use `manage_file` preview → apply and carry the returned active-file, file-state, output-state, or project-tree guards.
- **CBS / Danbooru validation:** load the matching syntax skill first, locate text with facade search/read, and validate before changing it with `validate_content` when selectors include CBS text or Danbooru tags; use granular `validate_cbs`, `simulate_cbs`, `diff_cbs`, `validate_danbooru_tags`, or `search_danbooru_tags` for simulation, diff, search, or exact legacy payloads. Re-run the same validators and preview/render if visible output can change.
...
3. For covered active/external field write/replace, active/external surface patch/replace, active/external lorebook text replacement or item write/delete, active/external regex/greeting item write/delete, active/external trigger write/delete, active/external Lua/CSS section write/replace/insert, external Lua/CSS section delete, covered external `.risup` prompt item write/delete operations, supported active/external `.risup` prompt batch operations, supported active regex/greeting prompt batch writes, or supported active greeting batch deletes, use `preview_edit` → `apply_edit`. For covered `.risup` prompt add/reorder/import/copy/snippet workflows and lorebook/regex/alternate greeting/trigger/Lua/CSS add/reorder, use `manage_items`; for covered asset add/delete/rename/compression, use `manage_assets`; for covered file/session/project-folder workflows, use `manage_file`; use granular routes for unsupported batch/block/add/reorder/asset/filesystem semantics.
...
3. Use facade `preview_edit` → `apply_edit` for covered active or unopened-file JSON Patch operations.
4. Use facade `preview_edit` / `apply_edit` for covered surface patch or recursive string replacement under a path. Use granular `patch_surface` / `replace_in_surface` / `external_patch_surface` only for exact legacy payloads, unsupported patch workflows, or cross-surface diagnostics.
5. Use `dry_run: true` before broad granular edits and pass `expected_hash` from `list_surfaces`, `read_surface`, or the facade preview guard when retry safety matters.
6. After active-document surface edits that must persist immediately, prefer `manage_file` preview/apply `save_current_file`.
