...
`npm run validate` runs environment preflight and the quick code feedback loop. Add `-- --plan --json` to inspect its checks without executing them, or `-- --test src/lib/mcp-search.test.ts` to focus unit tests. `npm run validate -- --profile mcp` shares one build across MCP integration, replay, and contracts; `npm run validate:full` includes tests and platform builds. Each run writes step logs and a JSON report under `.build/validation/`; inspect the failed step rather than rerunning successful checks blindly. `npm run doctor` diagnoses prerequisites; the optional `test:desktop` and `dev:isolated` commands cover real application startup as described in `CONTRIBUTING.md` and require working Chromium IPC.
