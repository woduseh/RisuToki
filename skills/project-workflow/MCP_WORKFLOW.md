...
> Canonical source: `docs/MCP_WORKFLOW.md`
> 도구 패밀리와 경계 정의: `docs/MCP_TOOL_SURFACE.md`
> 에러/no-op/성공 응답 계약: `docs/MCP_ERROR_CONTRACT.md`
...
- `list_risup_prompt_items` / `read_risup_prompt_item` 응답에는 additive `id` 필드가, `read_risup_formating_order` 응답에는 advisory `warnings` 배열이 포함됩니다.
...
- 프리뷰 패널은 초기화·런타임 진단을 인라인 배너로 표시합니다. iframe이 5초 안에 준비되지 않으면 타임아웃 에러를, Lua 트리거 실패 등 런타임 에러가 발생하면 해당 메시지를 패널 안에서 바로 볼 수 있습니다.
- 프리뷰는 현재 `.charx` 파일에서만 열립니다. `.risum` / `.risup`가 열려 있으면 프리뷰 경로가 차단됩니다.
- 프리뷰 Lua의 `setDescription`, `setPersonality`, `setScenario`, `setFirstMessage`는 preview-local 상태를 즉시 갱신합니다.
- 프리뷰 매크로는 `{{charpersona}}`(personality)와 `{{chardesc}}`(description)를 서로 다른 필드로 유지해야 합니다.

### MCP 분류 체계

- `src/lib/mcp-tool-taxonomy.ts`가 120개 도구를 19개 패밀리로 분류하는 단일 소스 오브 트루스입니다. 도구 추가/삭제 시 이 파일도 함께 갱신해야 하며, `mcp-tool-taxonomy.test.ts`가 양방향 완전성을 검증합니다.
...
> MCP 에러/no-op/성공 응답 계약의 상세 내용은 `docs/MCP_ERROR_CONTRACT.md`를 참고하세요.
...
- `read_skill("project-workflow")` — 프로젝트 온보딩, MCP 워크플로 요약, 프로젝트 규칙
...
`list_skills`는 각 스킬에 대해 `name`, `description`, `tags`, `relatedTools`, `files` 메타데이터를 반환합니다.
