...
**Hidden deprecated fields:** normal MCP read/search/probe/surface/facade routes hide deprecated `.charx` fields, `.risup` legacy prompt fields, reserved `.risum` `cjs`, and `groupOnlyGreetings` content. `inspect_document`, `list_fields`, `list_surfaces`, `session_status`, and external/reference inventories may return value-safe `hiddenFieldWarnings` with field names and count/size only; they do not expose hidden content. `.risum` `lowLevelAccess` is not hidden and remains visible/editable.
...
| **Regex or greeting edits**   | `inspect_document`; then facade `read_content`, `list_regex` / `read_regex_batch`, or `list_greetings` / `read_greeting_batch` instead of dumping raw arrays                                                 | For single indexed write/delete and supported active batch edits use `preview_edit` → `apply_edit`; use granular dry-run/preview routes for broad text replacement  | Facade covered batches route to `write_regex_batch`, `batch_write_greeting`, or `batch_delete_greeting`; use `replace_in_regex` or add/reorder granular routes for unsupported workflows                     | Re-read the edited index or list after deletion, preview affected chat output when relevant, and confirm no stale-index `409` required a refresh                     |
| **`.risup` prompt edits**     | `inspect_document`, facade `read_content`, `list_risup_prompt_items`, `search_in_risup_prompt_items`, `read_risup_prompt_item_batch`, `read_risup_formating_order`, and `diff_risup_prompt` when available   | For single indexed prompt-item write/delete and supported active batch edits use `preview_edit` → `apply_edit`; use import/snippet `dry_run` before large imports   | Facade covered batches route to `write_risup_prompt_item_batch` or `batch_delete_risup_prompt_items`; use add/import/snippet granular tools for unsupported workflows                                        | `validate_content`, `validate_risup_prompt_import` with the same source text after imports, `diff_risup_prompt`, `read_risup_formating_order`, and targeted re-reads |
