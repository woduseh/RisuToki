...
- Facade v1 is the default for covered workflows: use `inspect_document`, `read_content`, `search_document`, `preview_edit` → `apply_edit`, `manage_items`, and `manage_assets` before legacy/granular inspect/read/search/write routes.
- Use granular tools only as advanced/legacy fallbacks for unsupported facade selectors/operations, exact structured editors, direct external mutations not covered by facade, large exact exports, asset compression or unsupported asset workflows, unsupported batch workflows, or compatibility/debugging. Stable by-id/by-identity granular tools are preferred fallbacks when facade cannot express the operation.
...
- For active/external `.charx` or `.risum` assets, use `manage_assets` for covered list/read/add/delete workflows plus `.charx` rename. Use granular asset tools for compression, unsupported `.risum` rename, exact legacy response shapes, or low-level asset diagnostics.
- For unopened files, start with facade `inspect_document` / `read_content` when covered; use `preview_edit` → `apply_edit` for covered external field/surface, lorebook, regex, alternate greeting, trigger, Lua/CSS section, and `.risup` prompt item mutations; use `manage_assets` for covered external asset management. Use `inspect_external_file` + the relevant `probe_*` reader for probe-specific summaries, then use `external_search_in_field` / `external_read_field_range` / `external_write_field*` only when you need granular result shapes or direct external mutations.
...
| `.charx` / `.risum` asset list/read/add/delete and `.charx` asset rename on active or unopened external files                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  | `manage_assets`                  | You need asset compression, unsupported `.risum` rename, exact legacy response shapes, or raw asset/surface diagnostics.                                                                                           |
...
- **Asset edits:** inspect first, then use `manage_assets` read mode to list or read active/external `.charx` or `.risum` assets. For covered add/delete or `.charx` rename, use `manage_assets` preview → apply and carry `expected_asset_collection_digest`, `expected_path`, and external `expected_hash` guards. Use granular asset tools for compression, unsupported `.risum` rename, or exact legacy payloads.
...
- `manage_assets` for single covered asset add/delete/rename with preview/apply guards before falling back to granular asset routes
...
6. After active-document surface edits that must persist immediately, call `save_current_file`.
