...
| Active/external field write/replace, active surface patch, active lorebook text replace, active indexed regex/greeting/risup prompt item write/delete, covered active regex/greeting/risup prompt batch writes, or covered active greeting/risup prompt batch deletes | `preview_edit` then `apply_edit` | You need inserts, block replacements, unsupported batch semantics, add/reorder, snapshots, external surface patches, asset operations, item management, or unsupported patch semantics. |
...
- **Regex / greeting edits:** inspect first, then facade `read_content`, `list_regex` / `read_regex_batch`, or `list_greetings` / `read_greeting_batch`; avoid raw array dumps. Use `preview_edit` → `apply_edit` for covered single indexed writes/deletes and supported active batch writes/deletes, carrying `expected_comment` / `expected_preview` guards through the preview token. Use granular tools for regex batch deletes, add/reorder, or unsupported shapes, then re-read changed indices or list after deletion and preview affected output when relevant.
- **`.risup` prompt edits:** use facade `read_content`, `list_risup_prompt_items`, `search_in_risup_prompt_items`, `read_risup_prompt_item_batch`, `read_risup_formating_order`, and `diff_risup_prompt` before edits. Use `preview_edit` → `apply_edit` for covered single indexed prompt-item writes/deletes and supported active batch writes/deletes, carrying `expected_type` / `expected_preview`; prefer serializer/snippet dry-runs for large changes, granular import/snippet/add/reorder tools for unsupported workflows, then run `validate_content`, `validate_risup_prompt_import` after imports, and targeted prompt/order re-reads.
