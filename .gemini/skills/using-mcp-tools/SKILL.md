...
- Check tool `_meta` from `tools/list` when choosing a route or when the catalog facade is unavailable: `risutoki/profiles` and `risutoki/defaultProfile=facade-first` define the profile contract, `risutoki/surfaceKind=facade` plus `risutoki/recommendation=preferred` is the default for new covered workflows, `recommendation=advanced` marks granular escape hatches, and `recommendation=legacy` marks the documented Stage-1 compatibility inventory that new covered workflows should avoid. `risutoki/batchAlternative` points from single-item tools to sibling batch tools, `family` identifies the workflow family, `staleGuards` keeps the legacy flat guard-name list, `staleGuardDetails` gives guard `payloadPath`, list/read source operations, retry guidance, and batch alignment hints, `requiresConfirmation` means an approval gate is expected, and `supportsDryRun` means a preview-first flow exists.
...
Stage 1 entered soft legacy on 2026-07-03 after the measured replay passed 12/12 scenarios and 35/35 replayable tasks. The exact 51-entry inventory is canonical in `docs/MCP_TOOL_SURFACE.md`: one pre-existing compatibility facade (`load_guidance`) plus 50 covered granular routes across session, probes, reference readers, and indexed lorebook/regex/greeting/risup mutations. `legacy` is a routing signal, not removal: those tools remain available through their existing non-default profiles for exact compatibility/debug payloads and documented facade gaps. Known gaps still requiring advanced granular tools include unsupported batch structured item editors, unsupported trigger/Lua/CSS batch mutations, external block replacement, cross-surface replacement, non-artifact filesystem work, and broad validators without enough selector context.
...
- **Reference sync / diff:** use `analyze_content` with nested `{ kind: "reference", reference_id | file_path }` targets for lorebook and risup comparisons. The Stage-1 `list_reference_*` / `read_reference_*` inventory is for exact legacy payload compatibility, not the default route.
...
- **No main file required:** reference tools work even when no main document is open. Start with `inspect_document` and `read_content` targeting the session/reference. Use soft-legacy `session_status`, `list_references`, or `list_reference_*` / `read_reference_*` only when you need their exact compatibility payloads or runtime diagnostics.
