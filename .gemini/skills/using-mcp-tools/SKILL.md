...
| Active/external field write/replace, active/external surface patch, active/external lorebook text/id writes/deletes/replacements, active/external regex/greeting identity or index writes/deletes, active/external trigger writes/deletes, active/external Lua/CSS section writes/replacements/inserts, external Lua/CSS section deletes, active/external `.risup` prompt item id/index writes/deletes, older indexed regex/greeting/risup prompt item write/delete, covered active regex/greeting/risup prompt batch writes, covered active greeting deletes, or covered active/external `.risup` prompt batch writes/deletes | `preview_edit` then `apply_edit` | You need block replacements, unsupported batch semantics, add/reorder, snapshots, unsupported `.risum` asset compression, item management, non-patch surface operations, or unsupported patch semantics.           |
...
4. Use granular `patch_surface` / `external_patch_surface` only for exact legacy payloads or unsupported patch workflows; use `replace_in_surface` for recursive string replacement under a path.
