...
- Use `list_tool_profiles` for a compact profile-specific catalog (`facade-first`, `authoring`, `advanced-full`, `readonly`) while keeping `tools/list` unfiltered for compatibility. The catalog exposes runtime metadata, version-skew warnings, catalog health, and per-tool `workflowStages`; if it is unavailable, use tool `_meta` preference/profile hints from `tools/list`: `risutoki/profiles` declares membership, `risutoki/defaultProfile=facade-first` declares the planning default, and `risutoki/surfaceKind=facade` / `risutoki/recommendation=preferred` should win for covered workflows; granular tools default to `recommendation=advanced` unless a future migration marks them `legacy`.
...
- **Facade v1 (preferred)** — `list_tool_profiles`, `inspect_document`, `read_content`, `search_document`, `preview_edit`, `apply_edit`, `validate_content`, `load_guidance`
