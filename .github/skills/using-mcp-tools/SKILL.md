...
- Call `list_tool_profiles` when you need the active profile contract or another profile's catalog. `tools/list` returns only the registered startup profile: 13 bootstrap-capable `facade-first` tools by default, or `authoring`, `readonly`, or `advanced-full` (aliases `advanced` / `full`) when explicitly selected. This bounds eager-loading startup cost and remains compact for deferred-loading clients. Profile changes require an MCP server restart; catalogs do not expand the live tool set dynamically.
- Before a document is open, use `list_skills`, `read_skill`, or `load_guidance` with `{ kind: "guidance" }` for the catalog and add `skill` plus optional `document` for a specific guide. Document-dependent routes still return the structured `No file open` error.
...
- Facade v1 is additive and preferred where implemented. **Coverage rule:** if `preview_edit`, `manage_items`, `manage_assets`, or `manage_file` accepts the selector/operation in preview mode, the workflow is covered; apply through the returned `preview_token`, `operation_digest`, and guard values. If the facade rejects the selector or cannot express the operation, that rejection is the routing signal: restart with `advanced-full`, use the matching granular family, and state the unsupported facade selector or operation in one line of the final task summary. No separate log or commit-message record is required. The canonical per-route coverage matrix is the "First-wave facade replacement matrix" in `docs/MCP_TOOL_SURFACE.md`; the migration table below summarizes it. `read_content` defaults to a 24KB cap and returns root surface overviews unless `selector.include_raw=true` is paired with explicit `max_bytes`. Known facade gaps that still require granular tools: `.risum` asset compression, block-specific and cross-surface replacement workflows, CBS simulation/diff context, Danbooru search, exact legacy payloads, and oversize import/export text. Facade mutating flows are preview-token-first and must propagate stale guards from granular `risutoki/staleGuardDetails`. The measurable real-artifact routing matrix lives in `src/lib/mcp-agent-workflow-eval.test.ts`.
