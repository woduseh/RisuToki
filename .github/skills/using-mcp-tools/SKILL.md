...
- Use granular tools only as advanced/legacy fallbacks for unsupported facade selectors/operations, exact structured editors, direct external mutations, batch/add/reorder/import/export/asset workflows, or compatibility/debugging.
...
| Active/external field write/replace, active surface patch, active lorebook text replace, or active indexed regex/greeting/risup prompt item write/delete | `preview_edit` then `apply_edit` | You need inserts, block replacements, batch writes/deletes, add/reorder, snapshots, external surface patches, asset operations, item management, or unsupported patch semantics. |
...
Deprecation is staged and non-breaking today: current facade tools advertise `risutoki/surfaceKind=facade` and `risutoki/recommendation=preferred`; granular tools remain `surfaceKind=granular` with `recommendation=advanced` unless a future parity review marks a covered route `legacy`. Do not assume removal until a later warning window documents deprecation hints, first-party docs/evals no longer depend on the granular route, and release notes announce the change. Known gaps still requiring granular tools include validators outside active lorebook and `.risup` prompt/order structure checks, item/asset/file management, batch structured item editors, add/reorder, imports/exports/snippets, external surface patches, CBS/Danbooru validation, and broad batch operations. Post-v0.69.3 backlog: extend mutating facade parity beyond single indexed regex/greeting/risup prompt item writes/deletes into batch/add/reorder workflows where guard alignment is clear, and broaden `validate_content` only where facade selectors have clear validator parity. Track parity with `src/lib/mcp-agent-workflow-eval.test.ts`, `src/lib/mcp-request-schemas.test.ts`, `src/lib/mcp-tool-taxonomy.test.ts`, `src/lib/doc-drift.test.ts`, `test/test-mcp-search-all.ts`, and the matrix in `docs/MCP_TOOL_SURFACE.md`.
...
- **Regex / greeting edits:** inspect first, then facade `read_content`, `list_regex` / `read_regex_batch`, or `list_greetings` / `read_greeting_batch`; avoid raw array dumps. Use `preview_edit` → `apply_edit` for covered single indexed writes/deletes, and granular batch tools with `expected_comment` / `expected_preview` for multi-item changes, then re-read changed indices or list after deletion and preview affected output when relevant.
- **`.risup` prompt edits:** use facade `read_content`, `list_risup_prompt_items`, `search_in_risup_prompt_items`, `read_risup_prompt_item_batch`, `read_risup_formating_order`, and `diff_risup_prompt` before edits. Use `preview_edit` → `apply_edit` for covered single indexed prompt-item writes/deletes; prefer serializer/snippet dry-runs for large changes, granular batch/import/snippet tools carrying `expected_type` / `expected_preview`, then run `validate_content`, `validate_risup_prompt_import` after imports, and targeted prompt/order re-reads.
...
3. For covered active/external field write/replace, active surface patch, active lorebook text replacement, or active indexed regex/greeting/risup prompt item write/delete operations, use `preview_edit` → `apply_edit`; use granular routes for unsupported insert/batch/block/add/reorder semantics.
