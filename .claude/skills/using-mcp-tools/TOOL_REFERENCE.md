...
- Use `list_tool_profiles` for profile-specific catalogs (`facade-first`, `authoring`, `advanced-full`, `readonly`). The default `tools/list` contains 13 bootstrap-capable tools: 11 Facade v1 tools plus `list_skills` and `read_skill`. Changing profiles requires a server restart. The catalog exposes runtime metadata, version-skew warnings, catalog health, and per-tool `workflowStages`; use tool `_meta` preference/profile hints within the registered set when the catalog is unavailable.
...
- **Facade v1 (preferred)** — `list_tool_profiles`, `inspect_document`, `read_content`, `search_document`, `preview_edit`, `apply_edit`, `validate_content`, `load_guidance`, `manage_items`, `manage_assets`, `manage_file`
