...
- Facade v1 is additive and preferred where implemented: use `inspect_document`, `list_tool_profiles`, `read_content`, `search_document`, `preview_edit`, `apply_edit`, `validate_content`, and `load_guidance` for bounded facade/catalog workflows; keep granular tools as advanced/legacy routes for precision or unsupported cases. `read_content` defaults to a 24KB cap and returns root surface overviews unless `selector.include_raw=true` is paired with explicit `max_bytes`. `preview_edit` / `apply_edit` cover active/external field write/replace, active surface patch, active lorebook text replacement, active lorebook id writes/deletes, active regex/greeting identity writes/deletes, active/external `.risup` prompt-item id/index writes/deletes, older indexed regex/greeting/risup prompt item writes/deletes, active/external `.risup` prompt batch writes/deletes, active regex/greeting prompt batch writes, and active greeting batch deletes with derived stale guards. `validate_content` currently covers active lorebook key validation, regex compile/sanity checks, `.charx` export compatibility, CBS validation, Danbooru tag validation, active/external `.risup` promptTemplate checks, active `.risum` semantic field consistency, and external Plugin v3 source scans. `load_guidance` covers skill catalog/document reads including Plugin v3 guidance, and item/asset/file management remain future facade work. Facade mutating flows are preview-token-first (`preview_edit` before `apply_edit`) and must propagate stale guards from granular `risutoki/staleGuardDetails`. The measurable real-artifact routing matrix lives in `src/lib/mcp-agent-workflow-eval.test.ts`.
...
| Reference field reads and covered reference lorebook/regex/greeting/risup prompt selectors                                                                                                                                                                                                                                                                                                                                             | `read_content`                   | You need precision structured workflows, exact legacy payloads, batch item reads, delete/import/export workflows, or unsupported Lua/CSS/trigger reference families.                    |
...
| Active/external field write/replace, active surface patch, active lorebook text/id writes/deletes, active regex/greeting identity writes/deletes, active/external `.risup` prompt item id/index writes/deletes, older indexed regex/greeting/risup prompt item write/delete, covered active regex/greeting/risup prompt batch writes, covered active greeting deletes, or covered active/external `.risup` prompt batch writes/deletes | `preview_edit` then `apply_edit` | You need inserts, block replacements, unsupported batch semantics, add/reorder, snapshots, external surface patches, asset operations, item management, or unsupported patch semantics. |
...
3. For covered active/external field write/replace, active surface patch, active lorebook text replacement, active indexed regex/greeting/risup prompt item write/delete operations, covered external `.risup` prompt item write/delete operations, supported active/external `.risup` prompt batch operations, supported active regex/greeting prompt batch writes, or supported active greeting batch deletes, use `preview_edit` → `apply_edit`; use granular routes for unsupported insert/batch/block/add/reorder semantics.
