...
| ----------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Facade v1**                 | `list_tool_profiles`, `inspect_document`, `read_content`, `search_document`, `preview_edit`, `apply_edit`, `validate_content`, `load_guidance`                                                                                                                                                                                                                                                                                                                 | Preferred profile discovery plus bounded first-wave workflows for routed inspect/read/search and preview-token-first active field/surface edits                            |
...
| **Regex or greeting edits**   | `inspect_document`; then `list_regex` / `read_regex_batch` or `list_greetings` / `read_greeting_batch` instead of dumping raw arrays                                                                         | Use available dry-run/preview routes for broad text replacement; otherwise make the smallest indexed edit and carry stale guards                                                | `write_regex`, `replace_in_regex`, `write_regex_batch`, `batch_write_greeting`, or dedicated greeting write/delete tools with `expected_comment` / `expected_preview`                                          | Re-read the edited indices, preview affected chat output when relevant, and confirm no stale-index `409` required a refresh                                  |
| **`.risup` prompt edits**     | `list_risup_prompt_items`, `search_in_risup_prompt_items`, `read_risup_prompt_item_batch`, `read_risup_formating_order`, and `diff_risup_prompt` against references when available                           | `export_risup_prompt_to_text` / `copy_risup_prompt_items_as_text` for review; `import_risup_prompt_from_text(dry_run=true)` or snippet insertion `dry_run` before large imports | `write_risup_prompt_item_batch`, `add_risup_prompt_item_batch`, `batch_delete_risup_prompt_items`, `import_risup_prompt_from_text`, or `insert_risup_prompt_snippet` with `expected_type` / `expected_preview` | `validate_risup_prompt_import` with the same source text after imports, `diff_risup_prompt`, `read_risup_formating_order`, and targeted batch re-reads       |
