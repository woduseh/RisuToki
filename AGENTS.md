...
2. 항목이 많으면 `folder` 파라미터로 폴더별 필터링, `filter`로 키워드 검색
3. 먼저 `read_lorebook(index)`로 현재 내용 확인
4. `write_lorebook(index, data)` — **변경할 필드만** 전달 (부분 수정 가능)
5. comment 네이밍이 Lua 검색에 영향을 주므로 **기존 패턴과 일치**시킬 것

### 참고 자료 로어북 비교 시

1. `list_reference_lorebook(refIndex, filter?, folder?)` → 참고 파일 로어북 목록 확인
2. `read_reference_lorebook(refIndex, entryIndex)` → 개별 항목 읽기
3. `list_lorebook(filter?, folder?)` → 현재 파일 로어북 목록 확인
4. `read_lorebook(entryIndex)` → 현재 파일 개별 항목 읽기
5. 필요한 항목만 개별적으로 읽어서 비교 — 전체 덤프 금지
