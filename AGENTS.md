...
- **참고 자료의 lorebook/lua/css도 동일** — `read_reference_field("lorebook")`은 전체를 한번에 반환합니다. 대신:
  - 로어북: `list_reference_lorebook(index)` → `read_reference_lorebook(index, entryIndex)`
...
1. `list_lorebook`은 항목이 수백 개면 결과가 매우 클 수 있음 — **`filter` 파라미터로 검색 권장**
...
1. `list_reference_lorebook(refIndex, filter?)` → 참고 파일 로어북 목록 확인
2. `read_reference_lorebook(refIndex, entryIndex)` → 개별 항목 읽기
3. `list_lorebook(filter?)` → 현재 파일 로어북 목록 확인
