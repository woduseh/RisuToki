...
| `.risup` | 프리셋 (AI 모델/생성 파라미터)   | RPACK + 압축/암호화된 JSON               |
...
- 기타: `promptPreprocess`(boolean), `promptTemplate`(구조화 템플릿/JSON), `presetBias`(JSON), `formatingOrder`(구조화 순서/JSON), `autoSuggestPrompt`, `autoSuggestPrefix`, `autoSuggestClean`(boolean), `localStopStrings`(JSON), `outputImageModal`(boolean), `verbosity`(number), `fallbackWhenBlankResponse`(boolean), `systemContentReplacement`, `systemRoleReplacement`
...
### risup 프롬프트 템플릿 (Risup Prompt Template)

`.risup`의 `promptTemplate` / `formatingOrder`를 item-level로 다루는 전용 도구.
raw `read_field("promptTemplate")` / `write_field("promptTemplate")` / `read_field("formatingOrder")` / `write_field("formatingOrder")`는 fallback으로 유지되지만, 항목 단위 편집은 아래 도구를 우선 사용하세요.

| 도구                                   | 설명                                                                                                                                                                                              |
| -------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `list_risup_prompt_items()`            | `promptTemplate` 항목 목록 조회 (index, type, supported, preview). invalid JSON / non-risup이면 명시적 에러 반환                                                                                  |
| `read_risup_prompt_item(index)`        | 특정 인덱스의 prompt item 읽기. raw item object + supported/type metadata 반환                                                                                                                    |
| `write_risup_prompt_item(index, item)` | 특정 인덱스의 prompt item 교체. **지원되는 type만 허용** (`plain`, `jailbreak`, `cot`, `chatML`, `persona`, `description`, `lorebook`, `postEverything`, `memory`, `authornote`, `chat`, `cache`) |
| `add_risup_prompt_item(item)`          | 새 prompt item 추가. unsupported/raw shape는 거부되며 `write_field("promptTemplate")` fallback 사용                                                                                               |
| `delete_risup_prompt_item(index)`      | 특정 인덱스의 prompt item 삭제                                                                                                                                                                    |
| `reorder_risup_prompt_items(order)`    | prompt item 순서 변경. `order`는 현재 길이와 동일한 순열이어야 함                                                                                                                                 |
| `read_risup_formating_order()`         | `formatingOrder`를 `{ index, token, known }` 목록으로 읽기                                                                                                                                        |
| `write_risup_formating_order(items)`   | `formatingOrder` 쓰기. unknown **string** token은 허용/보존, non-string token은 명시적 에러                                                                                                       |
...
에디터 UI에서는 `.risup`를 charx/risum과 같은 범용 텍스트 항목으로 다루지 않고, **그룹화된 프리셋 폼 탭 + 설명 항목 + 정규식 폴더**로 노출합니다. Lua / 로어북 / 에셋은 risup 기본 편집 surface에서 제외됩니다.

visible `프롬프트` 그룹은 legacy `mainPrompt` / `jailbreak`가 아니라 `promptTemplate` / `formatingOrder` / `customPromptTemplateToggle` / 템플릿 변수 중심의 **template-first prompt surface**입니다. `promptTemplate`는 raw JSON textarea가 아니라 **구조화된 항목 목록 + 상세 편집기**로, `formatingOrder`는 **재정렬 가능한 토큰 리스트**로 열립니다. 손상된 JSON이 남아 있으면 복구용 raw editor와 validation error를 함께 표시합니다.

| 카테고리                | 주요 필드                                                                                                                                                        |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 모델                    | `aiModel`, `subModel`, `apiType`                                                                                                                                 |
| 프롬프트                | `promptTemplate`(구조화 편집 + raw fallback), `formatingOrder`(구조화 재정렬 + raw fallback), `customPromptTemplateToggle`(멀티라인), `templateDefaultVariables` |
| 레거시 프롬프트(호환성) | `mainPrompt`, `jailbreak`, `globalNote`, `useInstructPrompt`, `instructChatTemplate`, `JinjaTemplate`                                                            |
| 설명                    | `description`                                                                                                                                                    |
...
CBS, Lua, 로어북, 정규식, HTML/CSS, 트리거 스크립트 등의 상세 문법은 `list_skills` → `read_skill` MCP 도구로 on-demand 접근할 수 있습니다.

이 저장소는 번들 프로젝트 스킬 문서를 비워 둘 수 있으므로, `list_skills` 결과가 비어 있으면 로컬 `skills/` 폴더가 없는 상태로 간주하세요. 이 경우에는 `guides/`와 코드베이스 자체를 우선 참조하면 됩니다.
...
- **다음 필드는 `read_field`로 읽지 마세요** — 전체를 한번에 반환하여 컨텍스트를 낭비합니다:
  ...
  - `promptTemplate` / `formatingOrder` (risup item-level 작업) → `list_risup_prompt_items` / `read_risup_prompt_item` / `read_risup_formating_order`
...
- 실제 RisuAI `.risup` export는 gzip / zlib / raw-deflate 변형까지 호환되며, 저장 시 감지한 압축 모드를 최대한 보존함
- risup의 `promptTemplate` / `formatingOrder`는 구조화 UI와 전용 MCP 도구가 우선 surface이고, `customPromptTemplateToggle`는 멀티라인 textarea로 편집됨. legacy prompt 필드는 호환성 데이터로 유지되며, 잘못된 JSON이 남아 있으면 저장이 차단됨. unsupported raw shape를 직접 만져야 할 때만 `write_field` fallback 사용
...
- `skills/` — 선택적 로컬 프로젝트 스킬 문서 위치. 현재 저장소에서는 비어 있을 수 있으며, 이 경우 `list_skills` / `read_skill` 결과도 비어 있을 수 있음
...
- 이 `.github/agents/*.agent.md` 파일과 세션 `AGENTS.md`는 **런타임 산출물**이므로 소스 파일처럼 취급하거나 커밋하지 않습니다. 앱이 정리하지 못한 레거시 `.md` 파일도 다음 실행 시 안전하게 복원·제거됩니다.
