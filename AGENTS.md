...
- Copilot을 플루니 모드로 시작하면 선택한 카테고리(`solo`, `world-sim`, `multi-char`)에 맞춰 세션 `AGENTS.md`와 임시 `.github/agents/pluni.md`, `kotone.md`, `sophia.md`를 생성합니다.
- Copilot이 아닌 CLI에서는 같은 3인 자문 구조를 단일 세션 프롬프트로 합성해 전달합니다.
- 이 `.github/agents/*.md` 파일들은 런타임 산출물이므로 소스 파일처럼 취급하거나 커밋하지 않습니다.
