...
- **Bot composition:** `authoring-bots` holds project-specific formats and artifact placement; see `risu/bot/AGENTS.md`.
- **Shared CBS, lorebook, regex, Lua, HTML/CSS, trigger, asset, or file syntax:** `risu/common/AGENTS.md`.
- **Presets:** `prompt-family` covers the maintained Phēmē family; `writing-risup-presets` covers preset structure.
- **Modules or plugins:** see the corresponding router under `risu/modules` or `risu/plugins`.
