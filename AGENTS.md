...
- **Facade-first.** The default MCP process registers the Facade v1 tools (`list_tool_profiles`, `inspect_document`, `read_content`, `search_document`, `preview_edit` -> `apply_edit`, `validate_content`, `load_guidance`, `manage_items`, `manage_assets`, `manage_file`) plus bootstrap `list_skills` / `read_skill`. Coverage test: if the facade accepts your selector, the workflow is covered; if it rejects or cannot express it, restart the server with the smallest profile that covers the need (`authoring` for structured authoring families, `advanced-full` for the complete granular catalog) and fall back to the matching granular family. Record the fallback reason as one line in the final task summary, not in a separate log or commit message. Prefer stable `id` / `identity` selectors over indexes when item lists provide them.
...
- **Bounded reads.** `read_content` defaults to a 24KB cap and returns root surface overviews unless `include_raw` plus an explicit `max_bytes` are both justified.
