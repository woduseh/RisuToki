...
이 저장소는 번들 프로젝트 스킬 문서를 `skills/`에 포함합니다. `list_skills` 결과가 비어 있으면 로컬 `skills/` 폴더가 손실되었거나 링크 복구가 필요한 상태로 보고 `npm run sync:skills`와 작업 트리 상태를 먼저 확인하세요. 그래도 비어 있으면 `guides/`와 코드베이스 자체를 우선 참조하면 됩니다.
