...
Deprecated `.charx` fields, `.risup` legacy prompt fields, reserved `.risum` `cjs`, and `groupOnlyGreetings` are hidden from normal MCP/editor reads and searches. Use `hiddenFieldWarnings` only as a value-safe existence summary; do not route around the hidden policy to expose content. `.risum` `lowLevelAccess` remains visible/editable.
