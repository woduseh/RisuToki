...
Facade v1 MCP tools (`list_tool_profiles`, `inspect_document`, `read_content`, `search_document`, `preview_edit`, `apply_edit`, `validate_content`, `load_guidance`) are preferred for compact profile discovery plus bounded inspect/read/search/preview/apply/validate workflows when their selectors cover the task. Current facade mutations cover active/external field write/replace, active surface patch, active lorebook text/id writes/deletes, active regex/greeting identity writes/deletes, active `.risup` prompt item id writes/deletes, older indexed regex/greeting/risup prompt item writes/deletes, active regex/greeting/risup prompt batch writes, active greeting batch deletes, and active `.risup` prompt id batch writes/deletes. For new structured-item workflows, prefer `{ family: "risup-prompt", id }`, `{ family: "lorebook", id }`, and regex/greeting `identity` selectors before index selectors. `validate_content` covers lorebook, regex, `.charx` export compatibility via `family: "asset"` or `field: "exportCompatibility"`, CBS, Danbooru, `.risup`, `.risum`, and external Plugin v3 source-scan selectors where enough context is provided. Treat granular tools as advanced/legacy escape hatches for unsupported structured families, direct external surface mutation, broad unsupported batch/add/reorder/import/export/asset workflows, exact legacy response shapes, or precision/debug work.

Deprecated `.charx` fields, `.risup` legacy prompt fields, reserved `.risum` `cjs`, and `groupOnlyGreetings` are hidden from normal MCP/editor reads and searches and are removed from the saved file. Use `hiddenFieldWarnings` only as a value-safe existence summary; do not route around the hidden policy to expose content. `.risum` `lowLevelAccess` remains visible/editable.
