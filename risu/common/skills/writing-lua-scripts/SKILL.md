...
Load [`API_REFERENCE.md`](API_REFERENCE.md) only for exact function signatures, and the guide `risu/common/docs/문법가이드_Lua.md` only when Lua 5.4 language syntax itself is in question. For a genuinely mixed runtime-order problem, load [`RUNTIME_INTEROP.md`](../writing-trigger-scripts/RUNTIME_INTEROP.md). Continue the task using the corresponding Skills for CBS strings, HTML rendering, lorebook mechanics, or regex stages.
