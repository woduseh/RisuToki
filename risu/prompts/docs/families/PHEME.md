...
Use the current Phēmē document explicitly marked as the canonical Source and its prompt-item comments as the `.risup` synchronization contract. Provider or tool-call variants may diverge only where their request, caching, prefill, role, or output interface requires it.
