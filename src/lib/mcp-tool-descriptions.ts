export const MCP_TOOL_DESCRIPTIONS = {
  evaluate_bot:
    'Run repeatable, read-only bot regression cases against an active or external artifact. Checks text-field expectations, regex transformations, and lorebook activation. Each case requires expected results. Returns per-case assertions, pass counts, and a source fingerprint; rejects changes during the run. No LLM calls, roleplay-quality scoring, Lua execution, or writes. Use validate_content for syntax and format diagnostics.',
  inspect_document:
    'Start here to discover document fields, structured surfaces, references, runtime state, or authoring guides without editing. Examples: target={kind:"active"}, {kind:"external",file_path:"/absolute/path/bot.charx"}, or {kind:"guidance"}. Follow returned identities with read_content or search_document.',
  list_tool_profiles:
  ...
  analyze_content:
    'Analyze content without editing: field statistics, token counts, lorebook activation simulation, regex testing, CBS toggles/simulation, Danbooru vocabulary, and reference comparisons. Choose operation.action and its required arguments. Example: operation={action:"field_stats",field:"description"}. token_count requires explicit cl100k_base/o200k_base encoding and exactly one of text or selectors. Use verify_risup_prompt_import to check text import fidelity; use validate_content for syntax/semantic diagnostics.',
  validate_content:
    'Check selected syntax and semantic rules without editing. Select lorebook, regex, cbs, lua, trigger, danbooru, risup-prompt, risum, or external plugin-v3 content; supports .charx export compatibility. Lua/triggerlua checks compile without running scripts. Omitted selectors check lorebook keys only: select every desired family explicitly for broader checks. Simulation and comparisons belong to analyze_content.',
  load_guidance:
    'Legacy guidance loader. Prefer list_skills/read_skill, or inspect_document with target.kind="guidance" for guide documents. Reads the skill catalog or a skill document without editing.',
  preview_edit:
    'Preview targeted content writes, text replacement/insertion, structured item deletion, Lua/CSS edits, or JSON surface patches on active/external documents. Does not edit content. Each operation.op branch declares required fields: replace_text needs find (omitted replace means deletion), write_content/insert_text need content. Prefer stable id/identity selectors. Inspect the preview, then call apply_edit with its token, digest, target and guards. To add or reorder items use manage_items.',
  apply_edit:
    'Apply the exact mutation prepared by preview_edit. Copy preview_token, operation_digest, target and guard_values from the preview; do not resend or reconstruct operations. Requires user confirmation through the editor or standalone write gate. Tokens are single-use: after stale state, interruption, timeout or partial failure inspect the outcome and create a fresh preview before retrying. Verify changed content after success.',
  manage_items:
    'Add or reorder .risup prompt items, lorebook entries, regex entries, alternate greetings, triggers, or Lua/CSS sections on active/external documents. Also supports .risup text import, copy-as-text and reusable snippets. For snippet/copy reads use mode="read"; for mutations use mode="preview" (dry_run), inspect it, then mode="apply" with its token, digest and guard_values. For existing-item content edits or deletion use preview_edit/apply_edit. Requires user confirmation through the editor or standalone write gate.',
  manage_assets:
    'Preferred facade v1 asset-management tool for active or unopened external .charx/.risum assets. Use mode="read" for list/read, mode="preview" as the dry_run path for add/delete/rename/compress_assets mutations, then mode="apply" with the returned preview_token, operation_digest, and guard_values. Requires user confirmation on mutating apply. For add_asset prefer absolute source_path (max 8 MiB), or base64, never both; carry expected_source_hash from preview. WebP compression supports both families, updates .risum binary and extension metadata together, and preserves originals when conversion fails or grows.',
  manage_file:
    'Preferred facade v1 file-management tool for session-coupled file actions. Use mode="read" for snapshot/project-tree reads, mode="preview" as the dry_run path for create_document/open/save/snapshot/restore/field export/active lorebook import-export/project extract-reassemble mutations, then mode="apply" with preview_token, operation_digest, and guard_values. Requires user confirmation on mutating apply. create_document requires name and an absent absolute .charx/.risum/.risup path in an existing directory; works with no active document and never overwrites or switches documents. Supports active/session and explicit external paths.',
  list_fields:
