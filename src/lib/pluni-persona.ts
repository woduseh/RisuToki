...
/**
 * Build a compact advisor summary for embedding into session prompts.
 *
 * Intentionally leaner than the full markdown profile generated by
 * {@link buildAgentProfileMarkdown} in copilot-agent-profile-manager.ts.
 * The full profile includes toolkit, method, and deliverables; this summary
 * distills identity + tone + strengths + category focus to keep the session
...
export function buildAdvisorSummary(advisor: AdvisorId, category: ChatbotCategory): string {
  ...
  // First sentence of tone only — full tone lives in the agent profile markdown.
  const toneSnippet = profile.tone.split(/(?<=[.!?])\s/)[0] ?? profile.tone;
