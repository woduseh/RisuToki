...
const WORKFLOW_EVAL_TASKS = [
  ...
  },
  {
    ...
    route: {
      profile: 'authoring',
      discover: ['inspect_document', 'list_greetings'],
      readOrSearch: ['read_greeting_batch'],
      preview: [],
      apply: ['batch_write_greeting'],
      validate: ['read_greeting_batch'],
      granularFallbackReason: 'Greeting item mutation is outside the current facade edit scope.',
    },
    safety: {
      boundedOrItemizedRead: true,
      batchWhenSiblingItems: true,
      ...
      wrongTargetAvoidance: ['greeting_type must be explicit: alternate or group'],
      postEditValidation: ['batch re-read changed greeting indices'],
    },
    metrics: {
      routeCorrect: true,
      expectedFirstPassSuccess: true,
  ...
  },
  {
    ...
    route: {
      ...
      preview: [],
      apply: ['write_regex_batch'],
      validate: ['read_regex_batch'],
      granularFallbackReason: 'Regex item mutation is outside the current facade edit scope.',
    },
    safety: {
      boundedOrItemizedRead: true,
      batchWhenSiblingItems: true,
      staleGuards: ['expected_comment'],
      previewPolicy: 'not-supported',
      wrongTargetAvoidance: ['carry comments from list_regex/read_regex_batch into writes'],
      postEditValidation: ['batch re-read changed regex indices'],
    },
    metrics: {
      routeCorrect: true,
      expectedFirstPassSuccess: true,
  ...
  },
  {
    ...
    prompt: 'Patch a trigger script and the primary Lua section using section readers.',
    family: 'charx',
    corpusRoots: LOCAL_CORPUS_ROOTS.charx,
    sourceOfTruth: [UPSTREAM_RISUAI_SOURCE],
    ...
    route: {
      profile: 'authoring',
      discover: ['inspect_document', 'list_triggers', 'list_lua'],
      readOrSearch: ['read_trigger_batch', 'read_lua_batch'],
      preview: [],
      apply: ['write_trigger', 'write_lua'],
      validate: ['read_trigger_batch', 'read_lua_batch'],
      granularFallbackReason: 'Trigger and Lua section mutation require structured section tools.',
    },
    safety: {
      boundedOrItemizedRead: true,
      batchWhenSiblingItems: true,
      ...
      previewPolicy: 'not-supported',
      wrongTargetAvoidance: ['do not read or write raw triggerScripts/lua fields'],
      postEditValidation: ['section re-read plus stale 409 retry if hash/comment changed'],
    },
    metrics: {
      routeCorrect: true,
      expectedFirstPassSuccess: true,
  ...
  },
  {
    ...
    prompt: 'Patch a CSS section without dumping the full background field.',
    family: 'charx',
    corpusRoots: LOCAL_CORPUS_ROOTS.charx,
    sourceOfTruth: [UPSTREAM_RISUAI_SOURCE],
    surfaces: ['CSS'],
    ...
    route: {
      profile: 'authoring',
      discover: ['inspect_document', 'list_css'],
      readOrSearch: ['read_css'],
      preview: [],
      apply: ['write_css'],
      validate: ['read_css'],
      granularFallbackReason: 'CSS section mutation requires section hashes/previews.',
    },
    safety: {
      boundedOrItemizedRead: true,
      batchWhenSiblingItems: true,
      ...
      previewPolicy: 'not-supported',
      wrongTargetAvoidance: ['do not use read_field("css") or write_field("css")'],
      postEditValidation: ['CSS section re-read'],
    },
    metrics: {
      routeCorrect: true,
      expectedFirstPassSuccess: true,
  ...
  },
  {
    ...
    prompt: 'Rename or delete an embedded card asset only after identity inspection.',
    family: 'charx',
    corpusRoots: LOCAL_CORPUS_ROOTS.charx,
    sourceOfTruth: [UPSTREAM_RISUAI_SOURCE],
    surfaces: ['assets'],
    editRisk: 'destructive-edit',
    route: {
      profile: 'advanced-full',
      discover: ['inspect_document', 'list_charx_assets'],
  ...
  },
  {
    ...
    route: {
      profile: 'authoring',
      discover: ['inspect_document', 'list_risup_prompt_items'],
      readOrSearch: ['search_in_risup_prompt_items', 'read_risup_prompt_item_batch'],
      preview: [],
      apply: ['write_risup_prompt_item_batch'],
      validate: ['validate_content', 'read_risup_prompt_item_batch', 'read_risup_formating_order'],
      granularFallbackReason: 'Structured risup prompt item mutation remains a granular family.',
    },
    safety: {
      boundedOrItemizedRead: true,
      batchWhenSiblingItems: true,
      staleGuards: ['expected_type', 'expected_preview'],
      previewPolicy: 'not-supported',
      wrongTargetAvoidance: ['align expected_type/expected_preview with writes[].index'],
      postEditValidation: ['validate_content risup prompt/order check', 'batch item re-read'],
    },
    metrics: {
      routeCorrect: true,
      expectedFirstPassSuccess: true,
  ...
  },
  {
    ...
    route: {
      profile: 'authoring',
      discover: ['list_risup_prompt_items', 'list_risup_prompt_snippets'],
      readOrSearch: ['copy_risup_prompt_items_as_text', 'read_risup_prompt_snippet'],
      preview: ['insert_risup_prompt_snippet:dry_run'],
      apply: ['save_risup_prompt_snippet', 'insert_risup_prompt_snippet'],
      validate: ['read_risup_prompt_item_batch'],
      granularFallbackReason: 'Prompt snippet library operations are open-world granular tools.',
    },
    safety: {
      boundedOrItemizedRead: true,
      batchWhenSiblingItems: true,
  ...
  },
  {
    ...
    route: {
      profile: 'advanced-full',
      discover: ['inspect_document', 'list_references'],
      readOrSearch: ['export_risup_prompt_to_text', 'diff_risup_prompt'],
      preview: ['import_risup_prompt_from_text:dry_run'],
      apply: ['import_risup_prompt_from_text'],
      validate: ['validate_risup_prompt_import', 'diff_risup_prompt'],
      granularFallbackReason: 'Whole-template import/export/diff workflows are intentionally granular.',
    },
    safety: {
      boundedOrItemizedRead: true,
      batchWhenSiblingItems: true,
  ...
  },
  {
    ...
    prompt: 'Delete a module asset with expected_path protection and a post-delete list.',
    family: 'risum',
    corpusRoots: LOCAL_CORPUS_ROOTS.risum,
    sourceOfTruth: [UPSTREAM_RISUAI_SOURCE],
    surfaces: ['assets'],
    ...
    route: {
      profile: 'advanced-full',
      discover: ['inspect_document', 'list_risum_assets'],
      readOrSearch: ['read_risum_asset'],
      preview: [],
      apply: ['delete_risum_asset'],
      validate: ['list_risum_assets'],
      granularFallbackReason: 'Module asset management is outside the current facade scope.',
    },
    safety: {
      boundedOrItemizedRead: true,
      batchWhenSiblingItems: true,
      staleGuards: ['expected_path'],
      previewPolicy: 'not-supported',
      wrongTargetAvoidance: ['asset path from list_risum_assets must match delete_risum_asset expected_path'],
