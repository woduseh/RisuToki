 ...
 * @param {Buffer|Uint8Array} input - encoded bytes
 ...
 * @param {Buffer|Uint8Array} input - raw bytes
 ...
 * @param {Buffer} buf - raw module.risum bytes
 * @returns {{ module: object, assets: Buffer[] }}
 */
function parseRisum(buf) {
  let offset = 0;
  ...
  const mainJson = JSON.parse(mainDecoded.toString('utf-8'));
 ...
 * @param {object} moduleJson - the risuModule JSON object
 * @param {Buffer[]} [assets=[]] - optional embedded asset buffers
 * @returns {Buffer} risum binary
 */
function buildRisum(moduleJson, assets = []) {
